...
source = 'M87'
# How much flux we believe lives in the compact component (Jy)
zbl = 0.8
# True zero baseline values as measured by ALMA (Jy)
zbl_tot = 0.8
# Systematic noise added to visibilities to account for (e.g.,) leakage
sys_noise = 0.01
...
# Whether or not to include visibility amplitudes in the imaging
fit_amps = True
flag_amps = False            # Whether to flag anomalous amplitudes
snr_cut = 0                # SNR cutoff
t_avg = 0                # coherent averaging time (seconds)
# Gaussian FWHM for self-calibration of the LMT-SMT baseline (for M87)
LZ_gauss = 40 * eh.RADPERUAS
...
systematic_noise = {
    ...
    'PV': 0.05,
    'SMA': 0.01,
    'JCMT': 0.01}  # systematic noise on a priori amplitudes
# Image regularization parameters
reg_term = {'simple': 100, 'tv': 1, 'tv2': 1}
...
# setting zbl < zbl_tot assumes there is an extended constant flux
# component of zbl_tot-zbl Jy
...
# this seperate the data into scans, if it isn't done so already with an
# NX table
obs.add_scans()
...
# Order stations. This is to create a minimal set of closure quantities
...
# Add systematic noise for leakage (reminder: this must be done *after*
# any averaging)
...
# Helper function to repeat imaging with and without blurring to assure
# good convergence
...
# Make an image -- with closure quantities and visibility amplitudes.
# Initializing with a Gaussian Image
...
# Make an image -- with closure quantities and complex visibilities.
# Initializing with a Gaussian Image
...
if image_pol:
    ...
    if realdata:
        # FIX FOR ER5 DATA! rotate polarization because of error in ALMA ... do
        # not have to do this normally
        datadict = {t['site']: np.array(
            [(0.0, 0.0 + 1j * 1.0, 1.0 + 1j * 0.0)], dtype=eh.DTCAL) for t in obs_sc_pol.tarr}
        caltab = eh.caltable.Caltable(
            obs_sc_pol.ra,
            obs_sc_pol.dec,
            obs_sc_pol.rf,
            obs_sc_pol.bw,
            ...
            obs_sc_pol.source,
            obs_sc_pol.mjd)
        obs_sc_pol = caltab.applycal(
            obs_sc_pol, interp='nearest', extrapolate=True)
