...
################################################################################
        ...
        print ("blur_FWHM: " , beamparams[0]/eh.RADPERUAS)
        res = obs.res()
...
################################################################################
        ...
        chisqtab=r"\begin{tabular}{ l|l|l|l } \hline Triangle & $N_{cphase}$ & $\chi^2_{tri}$ & $\chi^2_{tri,red.}$\\ \hline \hline"
        first=True
        for i in range(len(cphase_chisq_data)):
            ...
            if first:
                chisqtab += r" " + tristr + " & " + nstr + " & " + chisqstr + " & " + rchisqstr 
                first=False
            else:
                chisqtab += r" \\" + tristr + " & " + nstr + " & " + chisqstr + " & " + rchisqstr 
...
################################################################################
        ...
        chisqtab=r"\begin{tabular}{ l|l|l|l } \hline Quadrangle & $N_{logcamp}$ & $\chi^2_{quad}$ & $\chi^2_{quad,red.}$ \\ \hline \hline"
...
################################################################################3
        ...
        chisqtab=r"\begin{tabular}{ l|l|l|l } \hline Baseline & $N_{amp}$ & $\chi^2_{amp}$ & $\chi^2_{amp,red.}$ \\ \hline \hline"
        ...
        if campplots:
            ...
            for quad in uniqueclosure_quad:
                ...
                ax = eh.plotting.comp_plots.plot_camp_obs_compare(obs_all,quad[0],quad[1],quad[2],quad[3],markersize=MARKERSIZE,
                                                                 camps=camps_all,ctype='logcamp',rangey=[-3,3],
                                                                 axis=ax,legend=False, clist=['k',SCOLORS[1]],
                                                                 ttype='nfft',show=False, ebar=ebar)
                if ax is None: continue
                if switch:
...
if __name__=='__main__':
    ...
    parser.add_argument("inputobs_uncal",help='/path/to/uvfits.fits')
    ...
    parser.add_argument('--systematic_cphase_noise', type=float, default=0)
