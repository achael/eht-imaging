...
def writeOIFITS(filename, RA, DEC, frequency, bandWidth, intTime,
	...
	for i in range(0, len(u)):
		...
		currVis = oifits.OI_VIS(timeobs[i], intTime, visamp[i], visamperr[i], visphi[i], visphierr[i], flagVis, u[i]*wavelength, v[i]*wavelength, data.wavelength['WAVELENGTH_NAME'], data.target[0], array=data.array['ARRAY_NAME'], station=station_curr, cflux=False, cfluxerr=False);
	...
	for j in range(0, len(uClosure)):
		...
		currT3 = oifits.OI_T3(timeClosure[j], intTime, t3amp[j], t3amperr[j], t3phi[j], t3phierr[j], flagVis, uClosure[j][0]*wavelength, vClosure[j][0]*wavelength, uClosure[j][1]*wavelength, vClosure[j][1]*wavelength, data.wavelength['WAVELENGTH_NAME'], data.target[0], array=data.array['ARRAY_NAME'], station=station_curr);
	...
	for k in range(0, len(u)):
		...
		currVis2 = oifits.OI_VIS2(timeobs[k], intTime, visamp[k]**2, 2.0*visamp[k]*visamperr[k], flagVis, u[k]*wavelength, v[k]*wavelength, data.wavelength['WAVELENGTH_NAME'], data.target[0], array=data.array['ARRAY_NAME'], station=station_curr);
