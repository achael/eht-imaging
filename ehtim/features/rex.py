...
class Profiles(object):
    ...
    def __init__(self, im, x0, y0, profs, thetas):
        ...
        self.im_center = im.shift(
            [int(np.round(deltay)), int(np.round(deltax))])
        ...
        self.interp = scipy.interpolate.interp2d(
            self.ys, self.xs, self.imarr, kind='cubic')
        ...
        self.RingSize1_med = (
            np.median(
                self.diameters), median_absolute_deviation(
                self.diameters))
    ...
    def calc_meanprof_and_stats(self):
        ...
        profile_peak_loc = np.unravel_index(
            np.argmax(self.profiles), self.profiles.shape)
        ...
        # profile avg inside ring
        self.in_level = np.mean(self.meanprof[0:inner_loc])

        # find outside mean flux
        outer_loc = np.argmin((self.rs - (RMAX - INNERTHRESH))**2)
        # profile avg outside ring
        self.out_level = np.mean(self.meanprof[outer_loc:])
        ...
        for i in range(self.nang):
            ...
            # AC ?? zero min profile before taking width???
        ...
        self.RingAngle1 = (
            scipy.stats.circmean(ringangles),
            scipy.stats.circstd(ringangles))
        ...
        self.RingContrast1 = np.max(
            self.profiles[:, self.lhloc:self.rhloc + 1]) / self.in_level
        ...
        # asymmetry 2: integrated flux in bottom half of ring vs top half of
        # ring
        ...
        # mask annulus
        rad_inner = (self.RingSize1[0] / 2. -
                     self.RingWidth[0] / 2.) * eh.RADPERUAS
        rad_outer = (self.RingSize1[0] / 2. +
                     self.RingWidth[0] / 2.) * eh.RADPERUAS

        mask_inner.imvec *= 0
        mask_outer.imvec *= 0
        mask_inner = mask_inner.add_gauss(
            1, [2 * rad_inner, 2 * rad_inner, 0, x0_c, y0_c])
        mask_inner = mask_inner.mask(cutoff=.5)
        mask_outer = mask_outer.add_gauss(
            1, [2 * rad_outer, 2 * rad_outer, 0, x0_c, y0_c])
        mask_outer = mask_outer.mask(cutoff=.5)

        maskvec_annulus = np.logical_xor(
            mask_inner.imvec.astype(bool),
            mask_outer.imvec.astype(bool))
        ...
        def anglemask(x, y):
            ang = np.mod(-np.arctan2(y - y0_c, x - x0_c) +
                         np.pi / 2., 2 * np.pi)
            # return ang
        ...
        self.RingAsym2 = ((brightflux - dimflux) /
                          (brightflux + dimflux), brightflux / dimflux)
    ...
    def plot_img(self, save_png=False):
        ...
        if save_png:
            ...
            fname = POSTPROCDIR + '/' + dirname + \
                '/' + basename[:-5] + '_contour.png'
    ...
    def plot_unwrapped(
        ...
        bandloc_sigma = np.sqrt(
            (self.RingWidth[1] / 2)**2 + (self.RingSize1[1] / 2)**2)

        rhloc = (self.RingSize1[0] / 2. + self.RingWidth[0] / 2.) * uas_to_pix
        rhloc2 = (
            self.RingSize1[0] / 2. + self.RingWidth[0] / 2. + bandloc_sigma) * uas_to_pix
        rhloc3 = (
            self.RingSize1[0] / 2. + self.RingWidth[0] / 2. - bandloc_sigma) * uas_to_pix
        ...
        plt.plot([self.abspk_loc_ang], [self.abspk_loc_rad],
                 'kx', mew=2, ms=6, color=pkcolor)
    ...
    def plot_prof_band(
        ...
        if cutloc1 < cutloc2:
            ...
            prof_half_2 = np.vstack(
                (self.profiles[cutloc2 + 1:], self.profiles[0:cutloc1]))
        else:
            prof_half_1 = np.vstack(
                (self.profiles[cutloc1:], self.profiles[0:cutloc2 + 1]))
    ...
    def plot_meanprof_theta(self, color='k', save_png=False):
        ...
        if save_png:
            ...
            basename = os.path.basename(self.imname)
            fname = POSTPROCDIR + '/' + dirname + '/' + \
                basename[:-5] + '_meanangprofile.png'
...
def compute_ring_profile(im, x0, y0, title="", nrays=NRAYS, nrs=NRS):
    ...
    imarr = im.imvec.reshape(im.ydim, im.xdim)[
        ::-1] * factor  # in brightness temperature K
...
def findCenter(im):
    ...
    def objFunc(pos):
        (x0, y0) = pos
        profiles = compute_ring_profile(
            im, x0, y0, nrs=NRS_SEARCH, nrays=NRAYS_SEARCH)
    ...
    res = scipy.optimize.brute(objFunc, ranges=(
        (fovmin_x, fovmax_x), (fovmin_y, fovmax_y)), Ns=NSEARCH)
    #print (time.time() - t)
...
def FindProfileSingle(
    ...
    with HiddenPrints():
        ...
        # compute profiles using the original (regridded, flux centroid
        # centered) image
        ...
        if save_files:
            ...
            basename = os.path.basename(imname)
            txtname = POSTPROCDIR + '/' + dirname + \
                '/' + basename[:-5] + tag + '.txt'
            radprof_name = POSTPROCDIR + '/' + dirname + \
                '/' + basename[:-5] + tag + '_radprof.txt'
            angprof_name = POSTPROCDIR + '/' + dirname + \
                '/' + basename[:-5] + tag + '_angprof.txt'

            fitsname = POSTPROCDIR + '/' + dirname + \
                '/' + basename[:-5] + tag + '.fits'
            ...
            f.write('meanprof_ring_diameter_sigma ' +
                    str(pp.RingSize2[1]) + '\n')

            f.write('ring_orientation: ' + str(pp.RingAngle1[0]) + '\n')
            f.write('ring_orientation_sigma: ' + str(pp.RingAngle1[1]) + '\n')

            f.write('meanprof_ring_orientation: ' +
                    str(pp.RingAngle2[0]) + '\n')
            f.write('meanprof_ring_orientation_sigma: ' +
                    str(pp.RingAngle2[1]) + '\n')
            ...
            f.write('ring_brighthalf_over_dimhalf ' +
                    str(pp.RingAsym2[1]) + '\n')
            ...
            f.write('ring_diameter_medabsdev ' +
                    str(pp.RingSize1_med[1]) + '\n')
...
def FindProfiles(
    ...
    arglist = [[imlist[i], save_files, blur, aipscc, tag, rerun, return_pp]
