...
##########################################################################
        ...
        ax.text(.23, .5, "%0.0f GHz" % (im.rf / 1.e9), fontsize=fs,
                ha='left', va='center', transform=ax.transAxes)
        ax.text(.23, .3, "%0.1f $\mu$as" % (im.fovx() / RADPERUAS),
                fontsize=fs, ha='left', va='center', transform=ax.transAxes)
...
##########################################################################
        ...
        for i in range(len(cphase_chisq_data)):
            ...
            if first:
                chisqtab += r" " + tristr + " & " + nstr + \
                    " & " + rchisqstr + " & " + rrchisqstr
                first = False
            else:
                chisqtab += r" \\" + tristr + " & " + nstr + \
                    " & " + rchisqstr + " & " + rrchisqstr
...
##########################################################################
        ...
        if snrcut_dict['logcamp'] > 0:
            ...
            mask = [camp['camp'] in camps_obs['camp']
                    for camp in camps_obs_all]
        ...
        for c in range(0, len(uniqueclosure_quad)):
            camps_obs_quad = obs.camp_quad(
                uniqueclosure_quad[c][0],
            ...
            if len(camps_obs_quad) > 0:
                camps_model_quad = obs.camp_quad(
                    uniqueclosure_quad[c][0],
                    uniqueclosure_quad[c][1],
                    uniqueclosure_quad[c][2],
                    uniqueclosure_quad[c][3],
...
# 3
        ...
        if gainplots:
            ...
            ax2 = ct_out.plot_gains(
                'all', rangey=[.1, 10], yscale='log', axis=ax2, legend=True, show=False)
...
# 3
        ...
        for ii in range(0, len(uniquebl)):
            ...
            amps_bl = obs.unpack_bl(
                bl[0], bl[1], [
                    'amp', 'sigma'], debias=debias)
            if len(amps_bl) > 0:

                amps_bl_model = obs_model.unpack_bl(
                    bl[0], bl[1], ['amp', 'sigma'], debias=False)

                if snrcut_dict['amp'] > 0:
                    amask = amps_bl['amp'] / \
                        amps_bl['sigma'] > snrcut_dict['amp']
...
def display_img(
    ...
    fov_scale = 40
    # fov_scale = int( math.ceil(fov_uas * roughfactor / 10.0 ) ) * 10 # round
    # around 1/3 the fov to nearest 10
