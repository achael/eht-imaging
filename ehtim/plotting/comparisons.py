    ...
    for fracidx in range(beam_steps):
        ...
        for i in range(len(imarr)):
            ...
            for j in range(i + 1, len(imarr)):
                ...
                (error,
                 im1_pad,
                 im2_shift) = img1.compare_images(img2,
                                                  metric=[metric],
                                                  psize=min_psize,
                                                  target_fov=max_fov,
                                                  blur_frac=0.0,
                ...
                if savepath:
                    im1_pad.save_fits(
                        savepath + '/' + str(i) + '_' + str(fracidx) + '.fits')
                    im2_shift.save_fits(
                        savepath + '/' + str(j) + '_' + str(fracidx) + '.fits')
...
# look over an array of images and determine the min pixel size and max
# fov that can be used consistently across them
def get_psize_fov(imarr):
    min_psize = 100
    for i in range(0, len(imarr)):
        if i == 0:
            max_fov = np.max([imarr[i].psize * imarr[i].xdim,
                              imarr[i].psize * imarr[i].ydim])
            min_psize = imarr[i].psize
        else:
            max_fov = np.max([max_fov,
                              imarr[i].psize * imarr[i].xdim,
                              imarr[i].psize * imarr[i].ydim])
...
def image_agreements(imarr, beamparams, metric_mtx, fracsteps, cutoff=0.95):
    ...
    for fracidx in range(len(fracsteps)):
        ...
        for c in range(len(cliques)):
            clique = cliques[c]
            im_avg = imarr[clique[0]].blur_gauss(
                beamparams, fracsteps[fracidx])

            for n in range(1, len(clique)):
                (error,
                 im_avg,
                 im2_shift) = im_avg.compare_images(imarr[clique[n]].blur_gauss(beamparams,
                                                                                fracsteps[fracidx]),
                                                    metric=['xcorr'],
                                                    psize=min_psize,
                                                    target_fov=max_fov,
...
def generate_consistency_plot(
        clique_fraclevels,
        im_clique_fraclevels,
        zoom=0.1,
        fov=1,
        show=True,
        framesize=(
    ...
    for c, column in enumerate(clique_fraclevels):
        ...
        for r, row in enumerate(column):
            ...
            arr_img = sample_image.imvec.reshape(
                sample_image.xdim, sample_image.ydim)
    ...
    ax.set_title('Blurred comparison of all images; cutoff={0}, fov (uas)={1}'.format(
        str(cutoff), str(im_clique_fraclevels[-1][-1].fovx() / eh.RADPERUAS)))
