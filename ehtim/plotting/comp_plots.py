...
def plotall_compare(obslist, imlist, field1, field2,
    ...
    (obslist_plot,
     clist_plot,
     legendlabels_plot,
     markers) = prep_plot_lists(obslist,
                                imlist,
                                clist=clist,
    ...
    for i in range(len(obslist_plot)):
        ...
        axis = obs.plotall(field1,
                           ...
                           legend=False,
                           snrcut=snrcut,
                           label=legendlabels_plot[i],
                           color=clist_plot[i % len(clist_plot)],
                           marker=markers[i],
...
def plot_cphase_compare(
    ...
    (obslist_plot, clist_plot, legendlabels_plot, markers) = prep_plot_lists(
        obslist, imlist, clist=clist, legendlabels=legendlabels, sgrscat=False, ttype=ttype)

    for i in range(len(obslist_plot)):
        obs = obslist_plot[i]
        axis = obs.plot_cphase(site1,
                               site2,
                               site3,
                               vtype=vtype,
...
def plotall_obs_im_cphases(obs,
                           imlist,
                           vtype='vis',
                           ang_unit='deg',
                           timetype='UTC',
    ...
    for image in imlist:
        obs_model = image.observe_same(
            obs, sgrscat=sgrscat, add_th_noise=False, ttype=ttype)
        cphases_model = obs_model.c_phases(
            mode='all', count='max', vtype=vtype)
    ...
    for c in range(0, len(uniqueclosure_tri)):
        cphases_obs_tri = obs.cphase_tri(
        ...
        if len(cphases_obs_tri) > 0:
            if print_chisqs:
                ...
                for i in range(1, len(obs_all)):
                    cphases_model_tri = obs_all[i].cphase_tri(
                        uniqueclosure_tri[c][0],
                        uniqueclosure_tri[c][1],
                        uniqueclosure_tri[c][2],
                        vtype=vtype,
                    ...
                    chisq_tri = np.sum((1.0 -
                                        np.cos(cphases_obs_tri['cphase'] *
                                               DEGREE -
                                               cphases_model_tri['cphase'] *
                                               DEGREE)) /
                                       ((cphases_obs_tri['sigmacp'] *
                                         DEGREE)**2))
            ...
            if display_mode == 'individual':
                ...
                ax = plt.subplot2grid(
                    (nrows, ncols), (nplot / ncols, nplot %
                                     ncols), fig=fig)
                axislabels = False

            f = plot_cphase_obs_compare(
                obs_all,
                uniqueclosure_tri[c][0],
                uniqueclosure_tri[c][1],
                uniqueclosure_tri[c][2],
...
def prep_plot_lists(
    ...
    if not((len(imlist) == len(obslist)) or len(
            imlist) <= 1 or len(obslist) <= 1):
        raise Exception(
            "imlist and obslist must be the same length, or either must have length 1")

    if not (
        legendlabels is None) and (
        len(legendlabels) != max(
            len(imlist),
            len(obslist))):
        raise Exception(
            "legendlabels should be the same length of the longer of imlist, obslist!")

    if legendlabels is None:
        legendlabels = [str(i + 1)
                        for i in range(max(len(imlist), len(obslist)))]
