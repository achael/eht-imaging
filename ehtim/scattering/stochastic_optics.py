...
# See http://adsabs.harvard.edu/abs/2016ApJ...833...74J for details about
# this module
...
class ScatteringModel(object):
    ...
    def __init__(
            ...
            model='dipole',
            scatt_alpha=1.38,
            observer_screen_distance=2.82 *
            3.086e21,
            source_screen_distance=5.53 *
            3.086e21,
        ...
        self.model = model
        # Major axis position angle [degrees, east of north]
        ...
        # Reference wavelength [cm]
        self.wavelength_reference = wavelength_reference_cm
        ...
        self.Qbar = 2.0 / sps.gamma((2.0 - self.scatt_alpha) / 2.0) * (self.r_in**2 * (1.0 + M) / (FWHM_fac * (self.wavelength_reference / (
            2.0 * np.pi))**2))**2 * ((theta_maj_mas_ref**2 + theta_min_mas_ref**2) * (1.0 / 1000.0 / 3600.0 * np.pi / 180.0)**2)
        self.C_scatt_0 = (self.wavelength_reference / (2.0 * np.pi))**2 * self.Qbar * \
            sps.gamma(1.0 - self.scatt_alpha / 2.0) / (8.0 * np.pi**2 * self.r_in**2)
        # Anisotropy, >=1, as lambda->infinity
        ...
        self.Amaj_0 = (self.r_in * (1.0 + M) * theta_maj_rad_ref / (FWHM_fac * \
                       (self.wavelength_reference / (2.0 * np.pi)) * 2.0 * np.pi))**2
        self.Amin_0 = (self.r_in * (1.0 + M) * theta_min_rad_ref / (FWHM_fac * \
                       (self.wavelength_reference / (2.0 * np.pi)) * 2.0 * np.pi))**2

        if model == 'von_Mises':
            def avM_Anisotropy(kzeta):
                return np.abs(
                    (kzeta * sps.i0(kzeta) / sps.i1(kzeta) - 1.0)**0.5 - A)
            ...
            def dipole_Anisotropy(kzeta):
                return np.abs(
                    sps.hyp2f1(
                        (self.scatt_alpha + 2.0) / 2.0, 0.5, 2.0, -kzeta) / sps.hyp2f1(
                        (self.scatt_alpha + 2.0) / 2.0, 1.5, 2.0, -kzeta) - A**2)
            ...
            self.kzeta = minimize(
                ...
                options={
                    'xtol': 1e-8,
                    'disp': False}).x
            self.P_phi_prefac = 1.0 / \
                (2.0 * np.pi * sps.hyp2f1((self.scatt_alpha + 2.0) / 2.0, 0.5, 1.0, -self.kzeta))
        else:
            print("Scattering Model Not Recognized!")

        # More parameters for the approximate phase structure function
        int_maj = integrate.quad(
            lambda phi_q: np.abs(
                np.cos(
                    self.phi0 -
                    phi_q))**self.scatt_alpha *
            self.P_phi(phi_q),
            0,
            2.0 *
            np.pi,
            limit=250)[0]
        int_min = integrate.quad(
            lambda phi_q: np.abs(
                np.sin(
                    self.phi0 -
                    phi_q))**self.scatt_alpha *
            ...
            np.pi,
            limit=250)[0]
        B_prefac = self.C_scatt_0 * 2.0**(2.0 - self.scatt_alpha) * np.pi**0.5 / (
            self.scatt_alpha * sps.gamma((self.scatt_alpha + 1.0) / 2.0))
    ...
    def P_phi(self, phi):
        if self.model == 'von_Mises':
            return self.P_phi_prefac * \
                np.cosh(self.kzeta * np.cos(phi - self.phi0))
        elif self.model == 'boxcar':
            return self.P_phi_prefac * (1.0 - ((np.pi / (2.0 * (1.0 + self.kzeta)) < (phi - self.phi0) % np.pi) & (
                (phi - self.phi0) % np.pi < np.pi - np.pi / (2.0 * (1.0 + self.kzeta)))))
        elif self.model == 'dipole':
            return self.P_phi_prefac * \
                (1.0 + self.kzeta * np.sin(phi - self.phi0)**2)**(-(self.scatt_alpha + 2.0) / 2.0)
    ...
    def rF(self, wavelength):
        ...
        """
        return (self.source_screen_distance *
                self.observer_screen_distance /
                (self.source_screen_distance +
                 self.observer_screen_distance) *
                wavelength /
                (2.0 *
                 np.pi))**0.5
    ...
    def dDphi_dz(self, r, phi, phi_q, wavelength):
        ...
        """
        return 4.0 * (wavelength / self.wavelength_reference)**2 * self.C_scatt_0 / self.scatt_alpha * (
            sps.hyp1f1(-self.scatt_alpha / 2.0, 0.5, -r**2 / (4.0 * self.r_in**2) * np.cos(phi - phi_q)**2) - 1.0)
    ...
    def Dmaj(self, r, wavelength_cm):
        return (wavelength_cm / self.wavelength_reference)**2 * self.Bmaj_0 * (2.0 * self.Amaj_0 / (self.scatt_alpha * self.Bmaj_0))**(-self.scatt_alpha / (2.0 - self.scatt_alpha)
                                                                                                                                       ) * ((1.0 + (2.0 * self.Amaj_0 / (self.scatt_alpha * self.Bmaj_0))**(2.0 / (2.0 - self.scatt_alpha)) * (r / self.r_in)**2)**(self.scatt_alpha / 2.0) - 1.0)

    def Dmin(self, r, wavelength_cm):
        return (wavelength_cm / self.wavelength_reference)**2 * self.Bmin_0 * (2.0 * self.Amin_0 / (self.scatt_alpha * self.Bmin_0))**(-self.scatt_alpha / (2.0 - self.scatt_alpha)
                                                                                                                                       ) * ((1.0 + (2.0 * self.Amin_0 / (self.scatt_alpha * self.Bmin_0))**(2.0 / (2.0 - self.scatt_alpha)) * (r / self.r_in)**2)**(self.scatt_alpha / 2.0) - 1.0)
    ...
    def Dphi_approx(self, x, y, wavelength_cm):
        ...
        return (Dmaj_eval + Dmin_eval) / 2.0 + (Dmaj_eval - \
                Dmin_eval) / 2.0 * np.cos(2.0 * (phi - self.phi0))
    ...
    def Q(self, qx, qy):
        ...
        q = (qx**2 + qy**2)**0.5 + 1e-12 / self.r_in
        phi_q = np.arctan2(qy, qx)

        return self.Qbar * (q * self.r_in)**(-(self.scatt_alpha + 2.0)) * \
            np.exp(-(q * self.r_in)**2) * self.P_phi(phi_q)
    ...
    def sqrtQ_Matrix(
        ...
        # Derived parameters
        FOV = Reference_Image.psize * Reference_Image.xdim * \
        ...
        screen_x_offset_pixels = (Vx_km_per_s * 1.e5) * \
            (t_hr * 3600.0) / (FOV / float(N))
        screen_y_offset_pixels = (Vy_km_per_s * 1.e5) * \
            (t_hr * 3600.0) / (FOV / float(N))

        s, t = np.meshgrid(np.fft.fftfreq(N, d=1.0 / N),
                           np.fft.fftfreq(N, d=1.0 / N))
        sqrtQ = np.sqrt(self.Q(dq * s, dq * t)) * np.exp(2.0 * np.pi * 1j * \
                        (s * screen_x_offset_pixels + t * screen_y_offset_pixels) / float(N))
        sqrtQ[0][0] = 0.0  # A DC offset doesn't affect scattering

        return sqrtQ

    def Ensemble_Average_Kernel(
            self,
            Reference_Image,
            wavelength_cm=None,
        ...
        if wavelength_cm is None:
            wavelength_cm = C / Reference_Image.rf * \
                100.0  # Observing wavelength [cm]

        # assume square kernel.  FIXME: create ulist and vlist, and construct
        # u_grid and v_grid with the correct dimension
        ...
        if use_approximate_form:
            ...
            ker_uv = self.Ensemble_Average_Kernel_Visibility(
                u_grid, v_grid, wavelength_cm, use_approximate_form=use_approximate_form)
        else:
            ker_uv = np.array([[self.Ensemble_Average_Kernel_Visibility(
                u, v, wavelength_cm, use_approximate_form=use_approximate_form) for u in uvlist] for v in uvlist])

        ker = np.real(np.fft.fftshift(np.fft.fft2(ker_uv)))
        ker = ker / np.sum(ker)  # normalize to 1
        return ker

    def Ensemble_Average_Kernel_Visibility(
            self, u, v, wavelength_cm, use_approximate_form=True):
        """The ensemble-average multiplicative scattering kernel for visibilities at a particular {u,v} coordinate
           ...
           Returns:
            ...
            """
        if use_approximate_form:
            return np.exp(-0.5 * self.Dphi_approx(u * wavelength_cm / (1.0 + \
                          self.Mag()), v * wavelength_cm / (1.0 + self.Mag()), wavelength_cm))
        else:
            return np.exp(-0.5 * self.Dphi_exact(u * wavelength_cm / (1.0 + \
                          self.Mag()), v * wavelength_cm / (1.0 + self.Mag()), wavelength_cm))

    def Ensemble_Average_Blur(
            self,
            im,
            wavelength_cm=None,
            ker=None,
        ...
        if ker is None:
            ker = self.Ensemble_Average_Kernel(
                im, wavelength_cm, use_approximate_form)
    ...
    def Deblur_obs(self, obs, use_approximate_form=True):
        ...
        for i in range(len(vis)):
            ker = self.Ensemble_Average_Kernel_Visibility(
                u[i],
                v[i],
                wavelength_cm=C /
                obs.rf *
    ...
    def MakePhaseScreen(
        ...
        # Now we'll calculate the power spectrum for each pixel in Fourier
        # space
        screen_x_offset_pixels = (Vx_km_per_s * 1.e5) * \
            (t_hr * 3600.0) / (FOV / float(Nx))
        screen_y_offset_pixels = (Vy_km_per_s * 1.e5) * \
            (t_hr * 3600.0) / (FOV / float(Nx))

        if sqrtQ_init is None:
            sqrtQ = self.sqrtQ_Matrix(
                Reference_Image,
                Vx_km_per_s=Vx_km_per_s,
                Vy_km_per_s=Vy_km_per_s,
                t_hr=t_hr)
        else:
            # If a matrix for sqrtQ_init is passed, we still potentially need
            # to rotate it

            if screen_x_offset_pixels != 0.0 or screen_y_offset_pixels != 0.0:
                s, t = np.meshgrid(np.fft.fftfreq(
                    Nx, d=1.0 / Nx), np.fft.fftfreq(Ny, d=1.0 / Ny))
                sqrtQ = sqrtQ_init * np.exp(2.0 * np.pi * 1j * (
                    s * screen_x_offset_pixels + t * screen_y_offset_pixels) / float(Nx))
            else:
                sqrtQ = sqrtQ_init
        ...
        phi = np.real(
            wavelengthbar /
            FOV *
            EpsilonScreen.shape[0] *
            EpsilonScreen.shape[1] *
            np.fft.ifft2(
                sqrtQ *
                EpsilonScreen))
        phi_Image = image.Image(
            phi,
            Reference_Image.psize,
            Reference_Image.ra,
            Reference_Image.dec,
            rf=Reference_Image.rf,
            source=Reference_Image.source,
            mjd=Reference_Image.mjd)
    ...
    def Scatter(
        ...
        phi_Image = self.MakePhaseScreen(
            Epsilon_Screen,
            Unscattered_Image,
            obs_frequency_Hz,
            Vx_km_per_s=Vx_km_per_s,
        ...
        if Linearized_Approximation:
            ...
            EA_Gradient = Wrapped_Gradient(
                (EA_Image.imvec / (FOV / Nx)).reshape(EA_Image.ydim, EA_Image.xdim))
            ...
            if len(Unscattered_Image.qvec):
                # Scatter the Q image
                EA_Gradient = Wrapped_Gradient(
                    (EA_Image.qvec / (FOV / Nx)).reshape(EA_Image.ydim, EA_Image.xdim))
                EA_Gradient_x = -EA_Gradient[1]
                EA_Gradient_y = -EA_Gradient[0]
                ...
                # Scatter the U image
                EA_Gradient = Wrapped_Gradient(
                    (EA_Image.uvec / (FOV / Nx)).reshape(EA_Image.ydim, EA_Image.xdim))
                EA_Gradient_x = -EA_Gradient[1]
                EA_Gradient_y = -EA_Gradient[0]
            ...
            if len(Unscattered_Image.vvec):
                # Scatter the V image
                EA_Gradient = Wrapped_Gradient(
                    (EA_Image.vvec / (FOV / Nx)).reshape(EA_Image.ydim, EA_Image.xdim))
                EA_Gradient_x = -EA_Gradient[1]
                EA_Gradient_y = -EA_Gradient[0]
            ...
            for rx in range(Nx):
                for ry in range(Ny):
                    ...
                    # important.
                    rxp = int(np.round(
                        rx - rF**2.0 * phi_Gradient_x[ry, rx] / self.observer_screen_distance / Unscattered_Image.psize)) % Nx
                    ryp = int(np.round(
                        ry - rF**2.0 * phi_Gradient_y[ry, rx] / self.observer_screen_distance / Unscattered_Image.psize)) % Ny
    ...
    def Scatter_Movie(
        ...
        if not isinstance(
                Unscattered_Movie,
                movie.Movie) and framedur_sec is None:
            print(
                "If scattering a list of images of static image, the framedur must be specified!")
            return
        ...
        def get_frame(j):
            if isinstance(Unscattered_Movie, movie.Movie):
                im = image.Image(
                    Unscattered_Movie.frames[j].reshape(
                        (N, N)), psize, ra, dec, rf, pulse, source, mjd)
                if len(Unscattered_Movie.qframes) > 0:
                    im.add_qu(
                        Unscattered_Movie.qframes[j].reshape(
        ...
        scattered_im_List = [
            self.Scatter(
                ...
                Vx_km_per_s=Vx_km_per_s,
                Vy_km_per_s=Vy_km_per_s,
                t_hr=framedur_sec / 3600.0 * j,
                sqrtQ=sqrtQ,
                Linearized_Approximation=Linearized_Approximation,
                Force_Positivity=Force_Positivity) for j in range(N_frames)]

        if Return_Image_List:
            return scattered_im_List

        Scattered_Movie = movie.Movie(
            [
                im.imvec.reshape(
                    (im.xdim,
                     im.ydim)) for im in scattered_im_List],
            framedur=framedur_sec,
            psize=psize,
            ra=ra,
            dec=dec,
            rf=rf,
        ...
        if has_pol:
            Scattered_Movie_Q = [
                im.qvec.reshape(
                    (im.xdim, im.ydim)) for im in scattered_im_List]
            Scattered_Movie_U = [
                im.uvec.reshape(
                    (im.xdim, im.ydim)) for im in scattered_im_List]
...
def MakeEpsilonScreenFromList(EpsilonList, N):
    ...
    # This is because of conjugation symmetry in Fourier space to ensure a
...
def MakeEpsilonScreen(Nx, Ny, rngseed=0):
    ...
    epsilon = np.random.normal(loc=0.0,
                               ...
                               size=(Ny,
                                     Nx)) + 1j * np.random.normal(loc=0.0,
                                                                  scale=1.0 / math.sqrt(2),
                                                                  size=(Ny,
                                                                        Nx))
