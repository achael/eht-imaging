...
def leakage_cal(
    ...
    if not obs_test.frcal:
        print("Field rotation angles have not been corrected. Correcting now...")
        obs_test.data = simobs.apply_jones_inverse(
            obs_test, frcal=False, dcal=True, verbose=False)
        obs_test.frcal = True
    ...
    if len(sites) == 0:
        print(
            "No stations specified for leakage calibration: defaulting to calibrating all !")
        sites = allsites
    ...
    (dataRR, sigmaRR, ARR) = iu.chisqdata(obs, im_circ, mask=mask,
                                          dtype=dtype, pol='RR', ttype=ttype, fft_pad_factor=fft_pad_factor)
    (dataLL, sigmaLL, ALL) = iu.chisqdata(obs, im_circ, mask=mask,
                                          dtype=dtype, pol='LL', ttype=ttype, fft_pad_factor=fft_pad_factor)
    (dataRL, sigmaRL, ARL) = iu.chisqdata(obs, im_circ, mask=mask,
                                          dtype=dtype, pol='RL', ttype=ttype, fft_pad_factor=fft_pad_factor)
    (dataLR, sigmaLR, ALR) = iu.chisqdata(obs, im_circ, mask=mask,
                                          dtype=dtype, pol='LR', ttype=ttype, fft_pad_factor=fft_pad_factor)
    ...
    def chisq_total(data, im, D):
        if const_fpol:
            ...
            fpol_data_2 = 2.0 * \
                np.conj(data['lrvis'] / (data['rrvis'] + data['llvis']))
            fpol_sigma_1 = 2.0 / \
                np.abs(data['rrvis'] + data['llvis']) * data['rlsigma']
            fpol_sigma_2 = 2.0 / \
                np.abs(data['rrvis'] + data['llvis']) * data['lrsigma']
            ...
            if 'RR' in pol_fit:
                chisq_RR = iu.chisq(im.rrvec,
                                    ARR,
                                    obs_test.unpack_dat(data,
                                                        ['rr' + dtype])['rr' + dtype],
                                    data['rrsigma'],
                                    dtype=dtype,
                                    ttype=ttype,
    ...
    # Now, we will minimize the total chi-squared. We need two complex leakage
    # terms for each site
    optdict = {'maxiter': MAXIT}  # minimizer params
    Dpar_guess = np.zeros(
        (len(sites) + const_fpol) * 2,
        dtype=np.complex128).view(
        dtype=np.float64)
    print("Minimizing...")
    res = opt.minimize(
        errfunc,
    ...
    D_fit = res.x.astype(
        np.float64).view(
        dtype=np.complex128)  # all the D-terms (complex)
    ...
    obs_test.data = simobs.apply_jones_inverse(
        obs_test, dcal=False, verbose=False)
    obs_test.dcal = True

    if show_solution:
        print(
            "Original chi-squared: {:.4f}".format(
                chisq_total(
                    obs.switch_polrep('circ').data,
                    im_circ,
                    D_fit)))
        print(
        ...
        if const_fpol:
            print('Source Fractional Polarization Magnitude: {:.4f}'.format(
                np.abs(D_fit[-1])))
            print('Source Fractional Polarization EVPA [deg]: {:.4f}\n'.format(
                90. / np.pi * np.angle(D_fit[-1])))
...
def plot_leakage(
    ...
    for i in range(len(tarr)):
        ...
        label = tarr['site'][i]
        plt.hold(True)
        dre, = x.plot(np.real(tarr['dr'][i]) * 100.0, np.imag(tarr['dr'][i])
                      * 100.0, markers[0], markersize=markersize, color=color, label=label)
        dim, = x.plot(np.real(tarr['dl'][i]) * 100.0, np.imag(tarr['dl'][i])
                      * 100.0, markers[1], markersize=markersize, color=color, label=label)
...
#        plt.axes().set_aspect('equal', 'datalim')
    ...
    if axislabels:
        ...
        x.set_ylabel('Im[$D$] (%)')
    if legend:
        legend1 = plt.legend([l[0] for l in plot_points],
                             tarr['site'], ncol=1, loc=1)
