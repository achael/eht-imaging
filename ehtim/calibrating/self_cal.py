...
def self_cal(
    ...
    print(
        "Computing the Model Visibilities with " +
        ttype +
        " Fourier Transform...")
    obs_clean = im.observe_same_nonoise(
        obs, ttype=ttype, fft_pad_factor=fft_pad_factor)
    ...
    V_scans = [o[vis_poldict[pol]] for o in obs_clean.tlist(
    ...
    if processes > 0:  # run on multiple cores with multiprocessing
        scans_cal = np.array(pool.map(get_selfcal_scan_cal,
                                      [[i,
                                        len(scans),
                                          scans[i],
                                          im,
...
def self_cal_scan(
    ...
    def errfunc(gpar):
        ...
        chisq = np.sum(
            (verr.real * sigma_inv[nan_mask])**2) + np.sum((verr.imag * sigma_inv[nan_mask])**2)
    ...
    if caltable:
        ...
        for site in allsites:
            ...
            # self calibration, to avoid breaking polarization consistency
            # relationships
