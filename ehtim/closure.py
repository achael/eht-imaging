...
class Closure(object):
    ...
    def __init__(self, obs):
        ...
        # sites = [self.tarr[i][0] for i in range(len(self.tarr))] # List of
        # telescope names
        sites = [self.tarr[i][0]
                 for i in range(len(self.tarr))]  # List of telescope names
        bl = list(it.combinations(sites, 2))
        tris = list(it.combinations(sites, 3))
        quads = list(it.combinations(sites, 4))

        # closure phase/amp. time curves of all possible tri/quadr-angles
        # ("None" if no data)
    ...
    def record_cp(self, tri_id):
        ...
        fname = "cp_%s-%s-%s" % (self.tri[tri_id][0],
                                 self.tri[tri_id][1], self.tri[tri_id][2])
        f = open(fname, "w")
    ...
    def record_ca(self, quad_id):
        ...
        fname = "ca_%s-%s-%s-%s" % (self.quad[quad_id][0],
                                    self.quad[quad_id][1],
                                    self.quad[quad_id][2],
                                    self.quad[quad_id][3])
        f = open(fname, "w")
