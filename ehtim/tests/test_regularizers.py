...
# mask
mask = im.imvec > 0#0.1*np.mean(im2.imvec)
...
#im.imvec += 1e-3
#for j in range(len(im.imvec)):
#    im.imvec[j] *= (1.0 + (np.random.rand()-0.5)/10.0)
#    im.imvec[j] += (np.random.rand()/10.)*im.imvec[j]
...
for rtype in iu.REGULARIZERS:
    print(rtype,'norm_reg=False',iu.regularizer(blur_im.imvec, nprior, embed_mask, flux, im.xdim, im.ydim, im.psize, rtype, beam_size=20.*eh.RADPERUAS, norm_reg=False))
    print(rtype,'norm_reg=True',iu.regularizer(blur_im.imvec, nprior, embed_mask, flux, im.xdim, im.ydim, im.psize, rtype, beam_size=20.*eh.RADPERUAS, norm_reg=True))

for rtype in iu.REGULARIZERS:
    print('\nTesting the gradient of',rtype)
    def reg(imvec):
        return iu.regularizer(imvec, nprior, embed_mask, flux, im.xdim, im.ydim, im.psize, rtype, I_ref = I_ref, alpha_A = alpha_A, norm_reg=True)

    def reggrad(imvec):
        return iu.regularizergrad(imvec, nprior, embed_mask, flux, im.xdim, im.ydim, im.psize, rtype, I_ref = I_ref, alpha_A = alpha_A, norm_reg=True)
