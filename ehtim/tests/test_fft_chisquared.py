...
for dtype in ['vis', 'bs', 'amp', 'cphase', 'camp', 'logcamp']:
    ...
    chisqdata_fft = eh.imager.chisqdata(
        obs_dft,
        prior,
        mask,
        dtype,
    ...
    print("Fractional Difference direct-fast %0.4f" %
          np.abs((chisq_dft - chisq_fft) / (np.abs(chisq_dft))))
    print("Fractional Difference direct-nfft %0.4f" %
          np.abs((chisq_dft - chisq_nfft) / (np.abs(chisq_dft))))
    print("Fractional Difference nfft-fast %0.4f" %
          np.abs((chisq_nfft - chisq_fft) / (np.abs(chisq_nfft))))
...
for dtype in ['vis', 'bs', 'amp', 'cphase', 'camp', 'logcamp']:
    ...
    chisq_dft_grad = eh.imager.chisqgrad(
        ...
        ttype='direct',
        mask=mask)
    chisq_fft_grad = eh.imager.chisqgrad(
        test_imvec,
        chisqdata_fft[2],
        chisqdata_fft[0],
        chisqdata_fft[1],
    ...
    print("\n")
    print("Median Fractional Difference direct-fast %0.4f" %
          np.median(np.abs((chisq_dft_grad -
                            chisq_fft_grad) /
                           (np.abs(chisq_dft_grad) +
                            compare_floor))))
    print("Median Fractional Difference direct-nfft %0.4f" %
          np.median(np.abs((chisq_dft_grad -
                            chisq_nfft_grad) /
                           (np.abs(chisq_dft_grad) +
                            compare_floor))))
    print("Median Fractional Difference nfft-fast %0.4f" %
          np.median(np.abs((chisq_nfft_grad -
                            chisq_fft_grad) /
                           (np.abs(chisq_nfft_grad) +
                            compare_floor))))

    print("Max Fractional Difference direct-fast %0.4f" %
          np.max(np.abs((chisq_dft_grad -
                         chisq_fft_grad) /
                        (np.abs(chisq_dft_grad) +
                         compare_floor))))
    print("Max Fractional Difference direct-nfft %0.4f" %
          np.max(np.abs((chisq_dft_grad -
                         chisq_nfft_grad) /
                        (np.abs(chisq_dft_grad) +
                         compare_floor))))
    print("Max Fractional Difference nfft-fast %0.4f" %
          np.max(np.abs((chisq_nfft_grad -
                         chisq_fft_grad) /
                        (np.abs(chisq_nfft_grad) +
                         compare_floor))))
