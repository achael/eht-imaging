...
class Image(object):
    ...
    def ivec(self):
        if self.polrep != 'stokes':
            raise Exception(
                "ivec is not defined unless self.polrep=='stokes' -- try self.switch_polrep()")
    ...
    def qvec(self):
        if self.polrep != 'stokes':
            raise Exception(
                "qvec is not defined unless self.polrep=='stokes' -- try self.switch_polrep()")
    ...
    def uvec(self):
        if self.polrep != 'stokes':
            raise Exception(
                "uvec is not defined unless self.polrep=='stokes' -- try self.switch_polrep()")
    ...
    def vvec(self):
        if self.polrep != 'stokes':
            raise Exception(
                "vvec is not defined unless self.polrep=='stokes' -- try self.switch_polrep()")
    ...
    def rrvec(self):
        if self.polrep != 'circ':
            raise Exception(
                "rrvec is not defined unless self.polrep=='circ' -- try self.switch_polrep()")
    ...
    def rlvec(self):
        if self.polrep != 'circ':
            raise Exception(
                "rlvec is not defined unless self.polrep=='circ' -- try self.switch_polrep()")
    ...
    def lrvec(self):
        ...
        if self.polrep != 'circ':
            raise Exception(
                "lrvec is not defined unless self.polrep=='circ' -- try self.switch_polrep()")
    ...
    def add_pol_image(self, image, pol):
        ...
        if pol == self.pol_prim:
            raise Exception(
                "new pol in add_pol_image is the same as pol_prim!")
        if image.shape != (self.ydim, self.xdim):
            raise Exception(
                "add_pol_image image shapes incompatible with primary image!")
        if not (pol in list(self._imdict.keys())):
            raise Exception(
                "for polrep==%s, pol in add_pol_image in " %
                self.polrep +
                ",".join(
                    list(
                        self._imdict.keys())))
    ...
    def lin_polfrac(self):
        ...
        """
        if self.polrep == 'stokes':
            frac = np.abs(np.sum(self.qvec + 1j * self.uvec)) / \
                np.abs(np.sum(self.ivec))
        elif self.polrep == 'circ':
            frac = 2 * np.abs(np.sum(self.rlvec)) / \
    ...
    def circ_polfrac(self):
        ...
        if self.polrep == 'stokes':
            ...
            frac = np.sum(self.rrvec - self.llvec) / \
                np.abs(np.sum(self.rrvec + self.llvec))
    ...
    def centroid(self, pol=None):
        ...
        if len(imvec):
            xlist = np.arange(0, -self.xdim, -1) * pdim + \
                (pdim * self.xdim) / 2.0 - pdim / 2.0
            ylist = np.arange(0, -self.ydim, -1) * pdim + \
                (pdim * self.ydim) / 2.0 - pdim / 2.0
            x0 = np.sum(
                np.outer(
                    0.0 * ylist + 1.0,
                    xlist).ravel() * imvec) / np.sum(imvec)
            y0 = np.sum(np.outer(ylist, 0.0 * xlist + 1.0).ravel()
                        * imvec) / np.sum(imvec)
    ...
    def pad(self, fovx, fovy):
        ...
        for pol in list(self._imdict.keys()):
            ...
            if len(polvec):
                ...
                polarr = np.pad(
                    polarr, ((pady, pady), (padx, padx)), 'constant')
    ...
    def resample_square(self, xdim_new, ker_size=5):
        ...
        if self.xdim != self.ydim:
            raise Exception(
                "Image must be square to use Image.resample_square!")
        if self.pulse == ehtim.observing.pulses.deltaPulse2D:
            raise Exception(
                "Image.resample_squre only works on continuously parametrized images: does not work with delta pulses!")
        ...
        ij = np.array([[[i *
                         ...
                         2.0 -
                         self.psize /
                         2.0] for i in np.arange(0, -
                                                 self.xdim, -
                                                 1)] for j in np.arange(0, -
                                                                        self.ydim, -
                                                                        1)]).reshape((self.xdim *
                                                                                      self.ydim, 2))
        ...
        def im_new_val(imvec, x_idx, y_idx):
            x = x_idx * psize_new + \
                (psize_new * xdim_new) / 2.0 - psize_new / 2.0
            y = y_idx * psize_new + \
                (psize_new * ydim_new) / 2.0 - psize_new / 2.0
            mask = (((x -
                      ker_size *
                      self.psize /
                      2.0) < ij[:, 0]) *
                    ...
                    (ij[:, 1] < (y +
                                 ...
                                 self.psize /
                                 2.0))).flatten()
            interp = np.sum([imvec[n] * self.pulse(x - ij[n, 0], y - ij[n, 1],
                                                   self.psize, dom="I") for n in np.arange(len(imvec))[mask]])
            return interp
    ...
    def regrid_image(self, targetfov, npix, interp='linear'):
        ...
        def interp_imvec(imvec):
            if np.any(np.imag(imvec) != 0):
                return interp_imvec(np.real(imvec)) + 1j * \
                    interp_imvec(np.imag(imvec))
            #interpfunc = scipy.interpolate.RectBivariateSpline( y, x, np.reshape(imvec, (self.ydim, self.xdim) ) )
            interpfunc = scipy.interpolate.interp2d(
                y, x, np.reshape(imvec, (self.ydim, self.xdim)), kind=interp)
    ...
    def rotate(self, angle, interp='cubic'):
        ...
        def rot_imvec(imvec):
            if np.any(np.imag(imvec) != 0):
                return rot_imvec(np.real(imvec)) + 1j * \
                    rot_imvec(np.imag(imvec))
            imarr_rot = scipy.ndimage.interpolation.rotate(
                imvec.reshape(
                    (self.ydim,
                     self.xdim)),
                angle * 180.0 / np.pi,
    ...
    def blur_gauss(self, beamparams, frac=1., frac_pol=0):
        ...
        def gaussim(blurfrac):
            gauss = np.array([[np.exp(-(j * cth + i * sth)**2 / (2 * (blurfrac * sigma_maj)**2) - (
                i * cth - j * sth)**2 / (2. * (blurfrac * sigma_min)**2)) for i in xlist] for j in ylist])
    ...
    def blur_circ(self, fwhm_i, fwhm_pol=0):
        ...
        def blur(imarr, sigma):
            if np.any(np.imag(imarr) != 0):
                return blur(np.real(imarr), sigma) + 1j * \
                    blur(np.imag(imarr), sigma)
    ...
    def mask(self, cutoff=0.05, beamparams=None, frac=0.0):
        ...
        if beamparams is not None:
            ...
            if len(beamparams) == 3:
                ...
                raise Exception(
                    "beamparams should be a length 3 array [maj, min, posang]!")
        else:
            mask = self.copy()
    ...
    def apply_mask(self, mask_im, fill_val=0.):
        ...
        """
        if (self.psize != mask_im.psize) or (self.xdim !=
                                             mask_im.xdim) or (self.ydim != mask_im.ydim):
            raise Exception(
                "mask image does not match dimensions of the current image!")
    ...
    def add_flat(self, flux, pol=None):
        ...
        flatarr = ((flux / float(len(self.imvec))) *
                   np.ones(len(self.imvec))).reshape(self.ydim, self.xdim)
    ...
    def add_gauss(self, flux, beamparams, pol=None):
        ...
        gaussarr = np.array([[gaussian(i - x, j - y)
    ...
    def add_crescent(self, flux, Rp, Rn, a, b, x=0, y=0, pol=None):
        ...
        crescarr = np.array([[crescent(i - x, j - y)
                              for i in xlist] for j in ylist])
    ...
    def add_ring_m1(self, I0, I1, r0, phi, sigma, x=0, y=0, pol=None):
        ...
        def ringm1(x2, y2):
            if (x2**2 +
                ...
                flux = (I0 - 0.5 * I1 * (1 + np.cos(theta - phi))) / \
                    (2 * np.pi * r0)
                return flux
            else:
                return 0.0
    ...
    def add_const_pol(self, mag, angle, cmag=0, csign=1):
        ...
        if not (0 <= mag < 1):
            raise Exception(
                "fractional polarization magnitude must be between 0 and 1!")

        if not (0 <= cmag < 1):
            raise Exception(
                "circular polarization magnitude must be between 0 and 1!")
    ...
    def add_random_pol(self, mag, corr, cmag=0., ccorr=0., seed=0):
        ...
        sm = so.ScatteringModel(
            scatt_alpha=1.67,
            observer_screen_distance=dist,
            source_screen_distance=1.e5 * dist,
            theta_maj_mas_ref=theta_mas,
            theta_min_mas_ref=theta_mas,
            r_in=rdiff * 2,
            r_out=1e30)
        ep = so.MakeEpsilonScreen(im.xdim, im.ydim, rngseed=seed)
        ps = np.array(sm.MakePhaseScreen(
            ep, im, obs_frequency_Hz=29.979e9).imvec) / 1000**(1.66 / 2)
    ...
    def add_zblterm(
        ...
        print('Adding a ' +
              str(addedflux) +
              ' Jy circular Gaussian of FWHM size ' +
              str(gauss_sz /
                  RADPERUAS) +
              ' uas')
    ...
    def observe_same_nonoise(
        ...
        if (np.abs(self.ra - obs.ra) >
            ...
            raise Exception(
                "Image coordinates are not the same as observtion coordinates!")
        if (np.abs(self.rf - obs.rf) / obs.rf > tolerance):
            raise Exception(
                "Image frequency is not the same as observation frequency!")

        if ttype == 'direct' or ttype == 'fast' or ttype == 'nfft':
            print(
                "Producing clean visibilities from image with " +
                ttype +
                " FT . . . ")
        else:
            raise Exception(
    ...
    def observe_same(
        ...
        # NOTE There is an asymmetry here - in the old way, we don't offer the
        # ability to *not* unscale estimated noise.
        else:

            if caltable_path:
                print(
                    'WARNING: the caltable is currently only saved if you apply noise with a Jones Matrix')
    ...
    def compare_images(
        ...
        if im1.polrep != im2.polrep:
            raise Exception(
                "In find_shift, im1 and im2 must have the same polrep!")
        if im1.pol_prim != im2.pol_prim:
            raise Exception(
                "In find_shift, im1 and im2 must have the same pol_prim!")

        # Shift the comparison image to maximize normalized cross-corr.
        [idx,
         xcorr,
         im1_pad,
         im2_pad] = im1.find_shift(im2,
                                   psize=psize,
    ...
    def align_images(
        ...
        if not np.all(im0.polrep == np.array([im.polrep for im in im_list])):
            raise Exception(
                "In align_images, all images must have the same polrep!")
        if not np.all(im0.pol_prim == np.array(
                [im.pol_prim for im in im_list])):
            raise Exception(
                "In find_shift, all images must have the same pol_prim!")
        ...
        for i in range(0, len(im_list)):
            (idx,
             _,
             im0_pad_orig,
             im_pad) = im0.find_shift(im_list[i],
    ...
    def find_shift(self, im2, psize=None, target_fov=None,
        ...
        im1_norm = (
            im1_pad_vec.reshape(
                im1_pad.ydim,
                im1_pad.xdim) - np.mean(im1_pad_vec)) / np.std(im1_pad_vec)
        im2_norm = (
            im2_pad_vec.reshape(
                im2_pad.ydim,
                im2_pad.xdim) - np.mean(im2_pad_vec)) / np.std(im2_pad_vec)
    ...
    def hough_ring(
        ...
        if radius_range is None:
            hough_radii = np.arange(
                int(10 * RADPERUAS / self.psize), int(50 * RADPERUAS / self.psize))
        else:
        ...
        for accum, center_y, center_x, radius in zip(accums, cy, cx, radii):
            ...
            print(accum_frac)
            print(
                "%i ring diameter: %0.1f microarcsec" %
                (i, 2 * radius * pdim / RADPERUAS))
            if display_results:
                if i > len(colors):
                    color = colors[-1]
                else:
                ...
                circ = mpl.patches.Circle(
                    (center_y, center_x), radius, fill=False, color=color)
                ax.add_patch(circ)
            i += 1
    ...
    def fit_gauss(self, units='rad'):
        ...
        gauss_params = np.array((eig[0][1]**0.5 * (8. * np.log(2.))**0.5, eig[0][0]**0.5 * (
            8. * np.log(2.))**0.5, np.mod(np.arctan2(eig[1][1][0], eig[1][1][1]) + np.pi, np.pi)))
        if units == 'natural':
    ...
    def fit_gauss_empirical(self, paramguess=None):
        ...
        uv = np.array([[u, v] for u in np.arange(-u_max, u_max * 1.001, u_max / 4.0)
                       for v in np.arange(-u_max, u_max * 1.001, u_max / 4.0)])
        u = uv[:, 0]
        v = uv[:, 1]
        vis = np.dot(
            ehtim.obsdata.ftmatrix(
                self.psize,
                self.xdim,
                self.ydim,
                uv,
        ...
        optdict = {
            'maxiter': 5000,
            'maxfev': 5000,
            'xtol': paramguess[0] / 1e9,
            'ftol': 1e-10}  # minimizer params
        res = opt.minimize(
            errfunc,
            paramguess,
            method='Nelder-Mead',
    ...
    def contour(
        ...
        """Display the image in a contour plot.
           ...
           Args:
               ...
               cbar_unit (tuple of strings): the unit of each pixel for the colorbar: 'Jy', 'm-Jy', r'$\mu$Jy'
        ...
        for level in contour_levels:
            if not(contour_cfun is None):
                ...
                rgbstring = '#%02x%02x%02x' % (
                    rgbval[0] * 256, rgbval[1] * 256, rgbval[2] * 256)
            else:
                rgbstring = color
            cs = plt.contour(
                x,
    ...
    def display(
        ...
        """Display the image.
           ...
           Args:
               ...
               has_cbar (bool): True if you want a colorbar on the plot
               cbar_lims (tuple): specify the lower and upper limit of the colorbar
               cbar_unit (tuple): specifies the unit of the colorbar: eg ('Jy','pixel'),('m-Jy',r'$\mu$as$^2$'),['Tb'] (i.e., brightness temperature)
        ...
        if cbar_unit[0] in ['m-Jy', 'mJy']:
            ...
            if power != 0:
                fluxunit = (
                    r'Brightness Temperature ($10^{{' + str(power) + '}}$ K)')
            else:
                fluxunit = 'Brightness Temperature (K)'
        ...
        if len(cbar_unit) == 1 or cbar_unit[0] == 'Tb':
            ...
            if (beamparams is None or beamparams == False):
                ...
                areaunit = 'beam'
                beamarea = (2.0 * np.pi *
                            beamparams[0] * beamparams[1] / (8.0 * np.log(2)))
        ...
        if not plotp:
            ...
            try:
                ...
                try:
                    ...
                    imvec = np.array(
                        im2._imdict[pol]).reshape(-1) / (10.**power)
                except KeyError:
                    raise Exception(
                        "Cannot make pol %s image in display()!" %
                        pol)
            ...
            if has_title:
                plt.title(
                    "%s %.2f GHz %s" %
                    (self.source, self.rf / 1e9, pol), fontsize=16)
            ...
            if has_title:
                plt.title(
                    "%s %.2f GHz %s" %
                    (self.source, self.rf / 1e9, 'I'), fontsize=12)
        ...
        if label_type == 'ticks':
            ...
            end = start + fov_scale / fov_uas * self.xdim
            plt.plot([start, end], [self.ydim - start - 5, self.ydim -
                                    start - 5], color="white", lw=scale_lw)  # plot line
            plt.text(x=(start + end) / 2.0,
                     y=self.ydim - start + self.ydim / 30,
                     s=str(fov_scale) + r" $\mu$as",
                     color="white",
                     ha="center",
                     va="center",
    ...
    def overlay_display(self,
        ...
        (im_list_shift, shifts, im0_pad) = self.align_images(im_list, shift=shift,
                                                             final_fov=final_fov, scale=scale, gamma=gamma, dynamic_range=dynamic_range)

        unit = 'Jy/pixel'
        if scale == 'log':
            unit = 'log(Jy/pixel)'
            im0_pad.imvec = np.log10(
                im0_pad.imvec +
                np.max(
                    im0_pad.imvec) /
                dynamic_range[0])
            for i in range(0, len(im_list)):
                im_list_shift[i].imvec = np.log10(
                    im_list_shift[i].imvec + np.max(im_list_shift[i].imvec) / dynamic_range[i + 1])
        ...
        if scale == 'gamma':
            ...
            im0_pad.imvec = (
                ...
                np.max(
                    im0_pad.imvec) /
                dynamic_range[0])**(gamma)
            for i in range(0, len(im_list)):
                im_list_shift[i].imvec = (
                    im_list_shift[i].imvec + np.max(im_list_shift[i].imvec) / dynamic_range[i + 1])**(gamma)
        ...
        for i in range(-1, len(im_list)):
            ...
            if i == -1:
                ...
                immtx = im_list_shift[i].imvec.reshape(
                    im0_pad.ydim, im0_pad.xdim)
            ...
            for c in range(0, 3):
                composite_img[:, :, c] = composite_img[:,
                                                       :, c] + (color_coding[i + 1, c] * immtx)

        if not rescale:
            composite_img = composite_img - \
                np.min(np.min(np.min(composite_img)))
            composite_img = composite_img / \
                np.max(np.max(np.max(composite_img)))

        plt.subplot(111)
        plt.title(
            '%s   MJD %i  %.2f GHz' %
            (self.source, self.mjd, self.rf / 1e9), fontsize=20)
...
def load_image(image, display=False, aipscc=False):
    ...
    if isinstance(image, type("str")):
        if image.endswith('.fits'):
            ...
            print("Image format is not recognized. Was expecting .fits, .txt, or Image. Got <.{0}>. Returning False.".format(
                image.split('.')[-1]))
            return False
        ...
        print(
            "Image format is not recognized. Was expecting .fits, .txt, or Image. Got {0}. Returning False.".format(
                type(image)))
        return False
...
def load_txt(
    ...
    return ehtim.io.load.load_im_txt(
        fname,
        pulse=pulse,
        polrep=polrep,
        pol_prim=pol_prim,
...
def load_fits(fname, aipscc=False, pulse=PULSE_DEFAULT,
    ...
    return ehtim.io.load.load_im_fits(
        fname,
        aipscc=aipscc,
        pulse=pulse,
        polrep=polrep,
