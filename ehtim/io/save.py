...
def save_im_txt(im, fname, mjd=False, time=False):
    ...
    xs = np.array([[j for j in range(im.xdim)]
                   for i in range(im.ydim)]).reshape(im.xdim * im.ydim, 1)
    xs = pdimas * (xs[::-1] - im.xdim / 2.0)
    ys = np.array([[i for j in range(im.xdim)]
                   for i in range(im.ydim)]).reshape(im.xdim * im.ydim, 1)
...
def save_obs_txt(obs, fname):
    ...
    for i in range(len(obs.tarr)):
        head += (
            "%-8s %15.5f  %15.5f  %15.5f  %8.2f   %8.2f  %5.2f   %5.2f   %5.2f  %8.4f %8.4f %8.4f %8.4f \n" %
            (obs.tarr[i]['site'],
             obs.tarr[i]['x'],
             obs.tarr[i]['y'],
             obs.tarr[i]['z'],
    ...
    fmts = (
        "%011.8f %4.2f %6s %6s  %4.2f   %4.2f  %16.4f %16.4f    " +
...
def save_obs_uvfits(obs, fname, force_singlepol=None, polrep_out='circ'):
    ...
    # TODO necessary, or is it a pointer?
    hdulist_new['PRIMARY'].header = header
    ...
    col5 = fits.Column(
        ...
        unit='METERS',
        array=np.zeros(nsta))
    col6 = fits.Column(name='POLTYA', format='1A', array=np.array(
        ['R' for i in range(nsta)], dtype='|S1'))
    col7 = fits.Column(
        name='POLAA',
        format='1E',
        unit='DEGREES',
    ...
    col9 = fits.Column(name='POLTYB', format='1A', array=np.array(
        ['L' for i in range(nsta)], dtype='|S1'))
    col10 = fits.Column(
        name='POLAB',
        format='1E',
        unit='DEGREES',
    ...
    rdate_gstiao_out = rdate_tt_new.sidereal_time(
        'apparent', 'greenwich').degree
    rdate_offset_out = (
        rdate_tt_new.ut1.datetime.second -
        rdate_tt_new.utc.datetime.second)
    rdate_offset_out += 1.e-6 * \
        (rdate_tt_new.ut1.datetime.microsecond - rdate_tt_new.utc.datetime.microsecond)
    ...
    if (scan_arr is None or len(scan_arr) == 0):
        ...
        try:
            ...
            for scan in scan_arr:
                ...
                jd = round(fractimes[jj], ROUND_SCAN_INT) * \
                    comp_fac  # ANDREW TODO precision??
                ...
                if (np.floor(jd) >= np.floor(scan_start * comp_fac)
                    ...
                    scan_times.append(
                        scan_start + 0.5 * scan_dur)  # - rdate_jd_out)
                    scan_time_ints.append(scan_dur)
                    while (
                        jj < len(fractimes) and np.floor(
                            round(
                                fractimes[jj],
                                ROUND_SCAN_INT) *
                            comp_fac) <= np.ceil(
            ...
            if jj < len(fractimes):
                ...
                print(
                    "WARNING!!!: in save_uvfits NX table, didn't get to all entries when computing scan start/stop!")
                print (scan_times)
            time_nx = fits.Column(
                name="TIME",
                format="1D",
            ...
            timeint_nx = fits.Column(
                name="TIME INTERVAL",
                format="1E",
                unit='DAYS',
                array=np.array(scan_time_ints))
            sourceid_nx = fits.Column(
                name="SOURCE ID",
                format="1J",
                unit='',
                array=np.ones(
                    len(scan_times)))
            subarr_nx = fits.Column(
                name="SUBARRAY",
                format="1J",
                unit='',
                array=np.ones(
                    len(scan_times)))
            freqid_nx = fits.Column(
                name="FREQ ID",
                format="1J",
                unit='',
                array=np.ones(
                    len(scan_times)))
            startvis_nx = fits.Column(
                name="START VIS",
                format="1J",
                unit='',
                array=np.array(start_vis) + 1)
            endvis_nx = fits.Column(
                name="END VIS",
                format="1J",
                unit='',
                array=np.array(stop_vis) + 1)
            cols = fits.ColDefs([time_nx,
                                 timeint_nx,
                                 sourceid_nx,
                                 subarr_nx,
                                 freqid_nx,
...
def save_obs_oifits(obs, fname, flux=1.0):
    ...
    # Normalizing by the total flux passed in - note this is changing the data
    # inside the obs structure
    obs.data['vis'] /= flux
    obs.data['sigma'] /= flux

    data = obs.unpack(['u', 'v', 'amp', 'phase', 'sigma',
                       'time', 't1', 't2', 'tint'])
    ...
    antennaY = obs.tarr['y']
    antennaZ = obs.tarr['z']
    # antennaDiam = -np.ones(antennaX.shape) #todo: this is incorrect and
    # there is just a dummy variable here
    ...
    intTime = data['tint'][0]
    if not all(
        data['tint'][0] == item for item in np.reshape(
            data['tint'],
            (-1))):
        raise TypeError(
            "The time integrations for each visibility are different")
    ...
    dttime = np.array([datetime.datetime.utcfromtimestamp(x * 60.0 * 60.0)
                       for x in time])  # TODO: these do not correspond to the acutal times
    ...
    uClosure = np.transpose(
        np.array([np.array(biarr['u1']), np.array(biarr['u2'])]))
    vClosure = np.transpose(
        np.array([np.array(biarr['v1']), np.array(biarr['v2'])]))

    # convert times to datetime objects
    timeClosure = biarr['time']
    dttimeClosure = np.array([datetime.datetime.utcfromtimestamp(
        x * 60.0 * 60.0) for x in timeClosure])  # TODO: these do not correspond to the acutal times
    ...
    ehtim.io.writeData.writeOIFITS(
        fname,
        obs.ra,
        obs.dec,
        obs.rf,
    ...
    # Un-Normalizing by the total flux passed in - note this is changing the
    # data inside the obs structure back to what it originally was
    obs.data['vis'] *= flux
    obs.data['sigma'] *= flux
