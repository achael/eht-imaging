...
def writeOIFITS(
    ...
    name = 'TARGET_NAME'
    data.target = np.append(
        data.target, ehtim.io.oifits.OI_TARGET(
            name, RA, DEC, veltyp='LSR'))
...
# put in the wavelength information - only using a single frequency
    ...
    for i in range(0, len(antennaNames)):
        stations.append((antennaNames[i],
                         antennaNames[i],
                         i + 1,
                         antennaDiam[i],
                         [antennaX[i],
                          antennaY[i],
                          antennaZ[i]]))
    data.array['ARRAY_NAME'] = ehtim.io.oifits.OI_ARRAY(
        'GEOCENTRIC', [0, 0, 0], stations)
    ...
    # put in the visibility information - note this does not include phase
    # errors!
    for i in range(0, len(u)):
        station_curr = (data.array['ARRAY_NAME'].station[int(
            ant1[i] - 1)], data.array['ARRAY_NAME'].station[int(ant2[i] - 1)])
        currVis = ehtim.io.oifits.OI_VIS(
            timeobs[i],
            intTime,
            visamp[i],
            visamperr[i],
    ...
    for j in range(0, len(uClosure)):
        ...
        currT3 = ehtim.io.oifits.OI_T3(
            ...
            t3phierr[j],
            flagVis,
            uClosure[j][0] * wavelength,
            vClosure[j][0] * wavelength,
            uClosure[j][1] * wavelength,
            vClosure[j][1] * wavelength,
            data.wavelength['WAVELENGTH_NAME'],
            data.target[0],
            array=data.array['ARRAY_NAME'],
            station=station_curr)
    ...
    for k in range(0, len(u)):
        station_curr = (data.array['ARRAY_NAME'].station[int(
            ant1[k] - 1)], data.array['ARRAY_NAME'].station[int(ant2[k] - 1)])
        currVis2 = ehtim.io.oifits.OI_VIS2(
            timeobs[k],
            intTime,
            visamp[k]**2,
            2.0 * visamp[k] * visamperr[k],
            flagVis,
            u[k] * wavelength,
            v[k] * wavelength,
            data.wavelength['WAVELENGTH_NAME'],
