...
def linearized_bi(x0, A3, bispec, sigs, nPixels, alpha=100, reg="patch"):
    ...
    Alin, blin = computeLinTerms_bi(
        x0, A3, bispec, sigs, nPixels, alpha=alpha, reg=reg)
...
def linearizedSol_bs(
    ...
    Alin, blin = computeLinTerms_bi(
        currImage.imvec, A3, bispec, sigs, currImage.xdim * currImage.ydim, alpha=alpha, reg=reg)
...
def computeLinTerms_bi(x0, A3, bispec, sigs, nPixels, alpha=100, reg="patch"):
    ...
    blin = np.dot(np.transpose(np.dot(np.diag(alpha * sigmaR), dxReal)), (yR - betaR)) + \
        np.dot(np.transpose(np.dot(np.diag(alpha * sigmaI), dxImag)), (yI - betaI))
    Alin = np.dot(np.transpose(np.dot(np.diag(alpha * sigmaR), dxReal)), dxReal) + \
        np.dot(np.transpose(np.dot(np.diag(alpha * sigmaI), dxImag)), dxImag)
