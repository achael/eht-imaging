...
def pol_imager_func(Obsdata, InitIm, Prior,
    ...
    # mcv_transform = kwargs.get('logim',True) #transform m,chi with mcv (see
    # below)
    ...
    if pol_prim == "amp_phase":
        if len(
            InitIm.qvec) and (
            np.any(
                InitIm.qvec != 0) or np.any(
                InitIm.uvec != 0)):  # TODO right? or should it be if any=0
            init1 = (
                np.abs(
                    InitIm.qvec +
                    1j *
                    InitIm.uvec) /
                InitIm.imvec)[embed_mask]
            ...
            init1 = 0.2 * (np.ones(len(iimage)) + 1e-2 *
                           np.random.rand(len(iimage)))
    ...
    def objfunc(allvec):
        ...
        datterm = alpha_d1 * (chisq1(imtuple) - 1) + \
            alpha_d2 * (chisq2(imtuple) - 1)
    ...
    def objgrad(allvec):
        ...
        datterm = alpha_d1 * \
            chisq1grad(imtuple) + alpha_d2 * chisq2grad(imtuple)
    ...
    def plotcur(im_step):
        ...
        if show_updates:
            ...
            print(
                "i: %d chi2_1: %0.2f chi2_2: %0.2f s_1: %0.2f s_2: %0.2f" %
                (nit, chi2_1, chi2_2, s_1, s_2))
        nit += 1

    # Print stats
    print("Initial S_1: %f S_2: %f" % (reg1(inittuple), reg2(inittuple)))
    print("Initial Chi^2_1: %f Chi^2_2: %f" %
          (chisq1(inittuple), chisq2(inittuple)))
    if d1 in DATATERMS:
    ...
    optdict = {
        ...
        'maxls': MAXLS,
        'gtol': stop,
        'maxfun': 1.e100}  # minimizer dict params
    tstart = time.time()
    if grads:
        res = opt.minimize(
            objfunc,
    ...
    outim.add_qu(
        qimage.reshape(
            Prior.ydim, Prior.xdim), uimage.reshape(
            Prior.ydim, Prior.xdim))
...
def make_p_image(imtuple, pol_prim="amp_phase"):
    ...
    if pol_prim == "amp_phase":
        ...
        raise Exception(
            "polarimetric representation %s not recognized in make_p_image!" %
            pol_prim)
    return pimage
...
def make_m_image(imtuple, pol_prim="amp_phase"):
    ...
    if pol_prim == "amp_phase":
        ...
        raise Exception(
            "polarimetric representation %s not recognized in make_m_image!" %
            pol_prim)
    return mimage
...
def make_chi_image(imtuple, pol_prim="amp_phase"):
    ...
    if pol_prim == "amp_phase":
        ...
        raise Exception(
            "polarimetric representation %s not recognized in make_chi_image!" %
            pol_prim)
    return chiimage
...
def polchisqgrad(imtuple, A, data, sigma, dtype, ttype='direct',
    ...
    if ttype == 'direct':
        if dtype == 'pvis':
            chisqgrad = chisqgrad_p(
                imtuple, A, data, sigma, pol_prim, pol_solve)

        elif dtype == 'm':
            chisqgrad = chisqgrad_m(
                imtuple, A, data, sigma, pol_prim, pol_solve)

        elif dtype == 'pbs':
            chisqgrad = chisqgrad_pbs(
                imtuple, A, data, sigma, pol_prim, pol_solve)
        ...
        if dtype == 'pvis':
            chisqgrad = chisqgrad_p_nfft(
                imtuple, A, data, sigma, pol_prim, pol_solve)

        elif dtype == 'm':
            chisqgrad = chisqgrad_m_nfft(
                imtuple, A, data, sigma, pol_prim, pol_solve)

        elif dtype == 'pbs':
            chisqgrad = chisqgrad_pbs_nffts(
                imtuple, A, data, sigma, pol_prim, pol_solve)
...
def polchisqdata(Obsdata, Prior, mask, dtype, **kwargs):
    ...
    if ttype not in ['fast', 'direct', 'nfft']:
        raise Exception(
            "Possible ttype values are 'fast' and 'direct' and 'nfft'!")
    if ttype == 'direct':
        if dtype == 'pvis':
        ...
        if dtype == 'pvis':
            (data, sigma, A) = chisqdata_pvis_nfft(
                Obsdata, Prior, mask, **kwargs)
        elif dtype == 'm':
            (data, sigma, A) = chisqdata_m_nfft(Obsdata, Prior, mask, **kwargs)
        elif dtype == 'pbs':
            (data, sigma, A) = chisqdata_pbs_nfft(
                Obsdata, Prior, mask, **kwargs)
...
def chisqgrad_p(
    ...
    if pol_prim == "amp_phase":
        ...
        if pol_solve[0] != 0:
            gradi = -np.real(mimage * np.exp(-2j * chiimage)
                             * np.dot(Amatrix.conj().T, pdiff)) / len(p)
        else:
            gradi = zeros

        if pol_solve[1] != 0:
            gradm = -np.real(iimage * np.exp(-2j * chiimage)
                             * np.dot(Amatrix.conj().T, pdiff)) / len(p)
        else:
            gradm = zeros

        if pol_solve[2] != 0:
            gradchi = -2 * np.imag(pimage.conj() *
                                   np.dot(Amatrix.conj().T, pdiff)) / len(p)
        else:
            gradchi = zeros
        ...
        raise Exception(
            "polarimetric representation %s not added to pol gradient yet!" %
            pol_prim)
...
def chisqgrad_m(
    ...
    if pol_prim == "amp_phase":
        ...
        if pol_solve[0] != 0:
            gradi = (-np.real(mimage * np.exp(-2j * chiimage) * np.dot(Amatrix.conj().T, mdiff)) /
                     len(m) + np.real(np.dot(Amatrix.conj().T, msamples.conj() * mdiff)) / len(m))
        else:
            gradi = zeros

        if pol_solve[1] != 0:
            gradm = -np.real(iimage * np.exp(-2j * chiimage)
                             * np.dot(Amatrix.conj().T, mdiff)) / len(m)
        else:
            gradm = zeros
...
def chisq_pbs(imtuple, Amatrices, bis_p, sigma, pol_prim="amp_phase"):
    ...
    iimage = imtuple[0]
    pimage = make_p_image(imtuple, pol_prim)
    bisamples_p = np.dot(Amatrices[0],
                         pimage) * np.dot(Amatrices[1],
                                          pimage) * np.dot(Amatrices[2],
                                                           pimage)
    chisq = np.sum(np.abs((bis_p - bisamples_p) / sigma)**2) / \
        (2. * len(bis_p))
    return chisq
...
def chisqgrad_pbs(
    ...
    if pol_prim == "amp_phase":
        ...
        if pol_solve[0] != 0:
            gradi = -np.real(ptsum * mimage *
                             np.exp(2j * chiimage)) / len(bis_p)
        else:
            gradi = zeros
        if pol_solve[1] != 0:
            gradm = -np.real(ptsum * iimage *
                             np.exp(2j * chiimage)) / len(bis_p)
        else:
            gradm = zeros
        if pol_solve[2] != 0:
...
def chisqgrad_p_nfft(
    ...
    pdiff_vec = (-1.0 / len(p)) * (p - psamples) / \
        (sigmap**2) * pulsefac.conj()
    plan.f = pdiff_vec
    plan.adjoint()
...
def chisqgrad_m_nfft(
    ...
    if pol_prim == "amp_phase":
        ...
        msamples = psamples / isamples
        mdiff_vec = (-1. / len(m)) * (m - msamples) / \
            (isamples.conj() * sigmam**2) * pulsefac.conj()
        plan.f = mdiff_vec
        plan.adjoint()
...
def shw(imtuple, pol_prim="amp_phase"):
    ...
    iimage = imtuple[0]
    mimage = make_m_image(imtuple, pol_prim)
    S = -np.sum(iimage * (((1 + mimage) / 2) * np.log((1 + mimage) /
                                                      2) + ((1 - mimage) / 2) * np.log((1 - mimage) / 2)))
    return S
...
def stv_pol_grad(imtuple, nx, ny, pol_prim="amp_phase", pol_solve=(0, 1, 1)):
    ...
    if pol_prim == "amp_phase":
        ...
        # dS/dI Numerators
        if pol_solve[0] != 0:
            m1 = 2 * np.abs(im * im) - np.abs(im * im_l1) * np.cos(2 * (np.angle(im_l1) - np.angle(
                im))) - np.abs(im * im_l2) * np.cos(2 * (np.angle(im_l2) - np.angle(im)))
            m2 = np.abs(im * im) - np.abs(im * im_r1) * \
                np.cos(2 * (np.angle(im) - np.angle(im_r1)))
            m3 = np.abs(im * im) - np.abs(im * im_r2) * \
                np.cos(2 * (np.angle(im) - np.angle(im_r2)))
        ...
        # dS/dm numerators
        if pol_solve[1] != 0:
            m1 = 2 * np.abs(im) - np.abs(im_l1) * np.cos(2 * (np.angle(im_l1) - np.angle(im))
                                                         ) - np.abs(im_l2) * np.cos(2 * (np.angle(im_l2) - np.angle(im)))
            m2 = np.abs(im) - np.abs(im_r1) * \
                np.cos(2 * (np.angle(im) - np.angle(im_r1)))
            m3 = np.abs(im) - np.abs(im_r2) * \
                np.cos(2 * (np.angle(im) - np.angle(im_r2)))
        ...
        # dS/dchi numerators
        if pol_solve[2] != 0:
            c1 = -2 * np.abs(im * im_l1) * np.sin(2 * (np.angle(im_l1) - np.angle(im))) - \
                2 * np.abs(im * im_l2) * np.sin(2 * (np.angle(im_l2) - np.angle(im)))
            c2 = 2 * np.abs(im * im_r1) * np.sin(2 *
                                                 (np.angle(im) - np.angle(im_r1)))
            c3 = 2 * np.abs(im * im_r2) * np.sin(2 *
                                                 (np.angle(im) - np.angle(im_r2)))
...
def plot_m(imtuple, Prior, nit, chi2_dict, **kwargs):
    ...
    x = np.array([[i for i in range(Prior.xdim)]
                  for j in range(Prior.ydim)])[::thin, ::thin][mask2]
    y = np.array([[j for i in range(Prior.xdim)]
                  for j in range(Prior.ydim)])[::thin, ::thin][mask2]
    q = qimage(im, mim, chiim)
    u = uimage(im, mim, chiim)
    a = -np.sin(np.angle(q + 1j * u) / 2).reshape(Prior.ydim,
                                                  Prior.xdim)[::thin, ::thin][mask2]
    b = np.cos(
        np.angle(
            q +
            1j *
