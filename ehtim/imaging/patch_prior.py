...
def patchPrior(im, beta, patchPriorFile='naturalPrior.mat', patchSize=8):
    ...
    I1, counts = cleanImage(img, beta, nmodels, covs,
                            mixweights, means, patchSize)
...
def cleanImage(img, beta, nmodels, covs, mixweights, means, patchSize=8):
    ...
    cleanZ = cleanPatches(Z, patchSize, (beta)**(-0.5),
                          nmodels, covs, mixweights, means)
    ...
    I1 = np.transpose(np.bincount(np.array(
        list(map(int, np.reshape(temp, (-1))))), weights=np.reshape(cleanZ, (-1))))
    counts = np.transpose(np.bincount(np.array(list(map(int, np.reshape(
        temp, (-1))))), weights=np.reshape(np.ones(cleanZ.shape), (-1))))
...
def cleanPatches(Y, patchSize, noiseSD, nmodels, covs, mixweights, means):
    ...
    for i in range(0, nmodels):
        PYZ[i, :] = np.log(mixweights[i]) + loggausspdf2(Y,
                                                         covs[:, :, i] + SigmaNoise)
    ...
    for i in range(0, nmodels):
        ...
        Xhat[:,
             inds] = np.dot(covs[:,
                                 :,
                                 i],
                            np.dot(np.linalg.inv(covs[:,
                                                      :,
                                                      i] + SigmaNoise),
                                   Y[:,
                                     inds])) + np.dot(SigmaNoise,
                                                      np.dot(np.linalg.inv(covs[:,
                                                                                :,
                                                                                i] + SigmaNoise),
                                                             np.transpose(np.tile(np.transpose(means[:,
                                                                                                     i]),
                                                                                  [inds.shape[0],
                                                                                   1]))))
...
def loggausspdf2(X, sigma):
    ...
    q = np.sum((np.dot(np.linalg.inv(np.transpose(R)), X))
               ** 2, 0)  # quadratic term (M distance)
    # normalization constant
    c = d * np.log(2 * np.pi) + 2 * np.sum(np.log(np.diagonal(R)), 0)
    y = -(c + q) / 2.0
