...
# this means that pol imaging solves for m & chi (not I), for now
POL_SOLVE = (0, 1, 1)
...
class Imager(object):
    ...
    def __init__(
            ...
            prior_im=None,
            flux=None,
            data_term=DAT_DEFAULT,
            reg_term=REG_DEFAULT,
            **kwargs):
        ...
        # e.g. [('simple',1), ('l1',10), ('flux',500), ('cm',500)]
        ...
        self.systematic_cphase_noise_next = kwargs.get(
        ...
        self._fft_gridder_prad = kwargs.get(
            'fft_gridder_prad', GRIDDER_P_RAD_DEFAULT)
        self._fft_conv_func = kwargs.get(
            'fft_conv_func', GRIDDER_CONV_FUNC_DEFAULT)
        self._fft_pad_factor = kwargs.get('fft_pad_factor', FFT_PAD_DEFAULT)
        self._fft_interp_order = kwargs.get(
            'fft_interp_order', FFT_INTERP_DEFAULT)
    ...
    def make_image(self, pol=None, grads=True, **kwargs):
        ...
        if self.pol_next == 'P':
            ...
            self.prior_next = self.prior_next.switch_polrep(
                polrep_out='stokes', pol_prim_out='I')
            self.init_next = self.prior_next.switch_polrep(
                polrep_out='stokes', pol_prim_out='I')
            pol_prim = 'I'
        ...
        outim = image.Image(
            iimage_out.reshape(
                self.prior_next.ydim,
                self.prior_next.xdim),
            ...
            polrep=self.prior_next.polrep,
            pol_prim=pol_prim,
            mjd=self.prior_next.mjd,
            time=self.prior_next.time,
        ...
        for pol2 in list(outim._imdict.keys()):
            ...
            # Did we solve for polarimeric image or are we copying over old
            # pols?
        ...
        # Append to history
        # TODO - what should the log string be?
    ...
    def set_embed(self):
        ...
        if not np.any(self._embed_mask):
            raise Exception(
                "clipfloor_next too large: all prior pixels have been clipped!")

        coord = np.array([[[x, y] for x in np.arange(self.prior_next.xdim //
                                                     2, -
                                                     self.prior_next.xdim //
                                                     2, -
                                                     1)] for y in np.arange(self.prior_next.ydim //
                                                                            2, -
                                                                            self.prior_next.ydim //
                                                                            2, -
                                                                            1)])
        coord = self.prior_next.psize * \
            coord.reshape(self.prior_next.ydim * self.prior_next.xdim, 2)
    ...
    def check_params(self):
        ...
        if (self.prior_next.polrep == 'circ' and not(
                self.pol_next in ['P', 'RR', 'LL'])):
            raise Exception(
                "Initial image polrep is 'circ': pol_next must be 'RR' or 'LL' or 'P'!")

        if (self.prior_next.polrep == 'stokes' and not(
                self.pol_next in ['I', 'Q', 'U', 'V', 'P'])):
            raise Exception(
                "Initial image polrep is 'stokes': pol_next must be in 'I','Q','U','V','P'!")

        if (self.transform_next ==
                'log' and self.pol_next in ['Q', 'U', 'V', 'P']):
            raise Exception(
                "Cannot image Stokes Q, U, V, P with log image transformation!")
        ...
        if(self.pol_next in ['Q', 'U', 'V'] and
           ('gs' in self.reg_term_next.keys() or 'simple' in self.reg_term_next.keys())):
            raise Exception(
                "'simple' and 'gs' MEM methods do not work with potentially negative Stokes Q, U, or V images!")
        ...
        if (self.pol_next == 'P'):
            ...
            for term in sorted(self.dat_term_next.keys()):
                if (term is not None) and (term):
                    dt_here = True
                if not ((term in DATATERMS_POL) or term == False):
                    dt_type = False
            ...
            for term in sorted(self.reg_term_next.keys()):
                if (term is not None) and (term):
                    st_here = True
                if not ((term in REGULARIZERS_POL) or term == False):
                    st_type = False
            ...
            if not dt_type:
                raise Exception(
                    "Invalid data term for P imaging: valid data terms are: " +
                    ','.join(DATATERMS_POL))
            if not st_type:
                raise Exception(
                    "Invalid regularizer for P imaging: valid regularizers are: " +
                    ','.join(REGULARIZERS_POL))
            ...
            for term in sorted(self.dat_term_next.keys()):
                if (term is not None) and (term):
                    dt_here = True
                if not ((term in DATATERMS) or term == False):
                    dt_type = False
            ...
            for term in sorted(self.reg_term_next.keys()):
                if (term is not None) and (term):
                    st_here = True
                if not ((term in REGULARIZERS) or term == False):
                    st_type = False
        ...
        # determine if we need to recompute the saved imager parameters on the
        # next imager run
        if self.nruns == 0:
            return
    ...
    def check_limits(self):
        ...
        uvmin = 1.0 / (self.prior_next.psize *
                       np.max((self.prior_next.xdim, self.prior_next.ydim)))
        ...
        if uvmin > minbl:
            print(
                "Warning! Field of View is smaller than largest nonzero spatial wavelength!")
    ...
    def init_imager(self):
        ...
        if self.pol_next == 'P':  # polarimetric imaging
            ...
            # initialize m & phi
            if len(
                self.init_next.qvec) and (
                np.any(
                    self.init_next.qvec != 0) or np.any(
                    self.init_next.uvec != 0)):
                init1 = (
                    np.abs(
                        self.init_next.qvec +
                        1j *
                        self.init_next.uvec) /
                    self.init_next.imvec)[
                    self._embed_mask]
                init2 = (
                    np.arctan2(
                        self.init_next.uvec,
                ...
                print(
                    "No polarimetric image in init_next -- initializing with 10% pol and random orientation!")
                init1 = 0.2 * (np.ones(self._nimage) + 1e-2 *
                               np.random.rand(self._nimage))
                init2 = np.zeros(self._nimage) + 1e-2 * \
                    np.random.rand(self._nimage)
            ...
            if self.transform_next == 'mcv':
                ...
                raise Exception(
                    "Polarimetric imaging only works with transform_next='mcv'")
    ...
    def init_imager_scattering(self):
        ...
        self._sqrtQ = np.real(
            self.scattering_model.sqrtQ_Matrix(
                self.prior_next, t_hr=0.0))

        # Generate the initial image+screen vector. By default, the screen is
        # re-initialized to zero each time.
    ...
    def objfunc(self, imvec):
        ...
        if self.pol_next == 'P':
            imcur = unpack_poltuple(
                imvec, self._xtuple, self._nimage, POL_SOLVE)
        else:
            imcur = imvec
        ...
        for dname in sorted(self.dat_term_next.keys()):
            if self.chisq_transform:
                datterm += self.dat_term_next[dname] * (
                    chi2_term_dict[dname] + 1. / chi2_term_dict[dname] - 1.)
            else:
                datterm += self.dat_term_next[dname] * \
                    (chi2_term_dict[dname] - 1.)
        ...
        for dname in sorted(self.dat_term_next.keys()):
            ...
            if self.chisq_transform:
                datterm += self.dat_term_next[dname] * chi2_term_dict[dname] * (
                    1 - 1. / (chi2_value_dict[dname]**2))
            else:
                datterm += self.dat_term_next[dname] * \
                    (chi2_term_dict[dname] + self.chisq_offset_gradient)
    ...
    def plotcur(self, imvec, **kwargs):
        if self._show_updates:
            if self._nit % self._update_interval == 0:
                ...
                outstr += "\n%4d | " % self._nit
                for dname in sorted(self.dat_term_next.keys()):
                    outstr += "chi2_%s : %0.2f " % (dname,
                                                    chi2_term_dict[dname])

                outstr += "\n        "
                for dname in sorted(self.dat_term_next.keys()):
                    outstr += "%s : %0.1f " % (dname,
                                               chi2_term_dict[dname] * self.dat_term_next[dname])
                outstr += "\n        "
                for regname in sorted(self.reg_term_next.keys()):
                    outstr += "%s : %0.1f " % (regname,
                                               reg_term_dict[regname] * self.reg_term_next[regname])
    ...
    def objgrad_scattering(self, minvec):
        ...
        phi = self.scattering_model.MakePhaseScreen(
            ...
            sqrtQ_init=self._sqrtQ).imvec.reshape(
            (N,
             N))
        ...
        for regname in sorted(self.reg_term_next.keys()):
            ...
            if regname == 'rgauss':
                # get gradient of the scattered image vector
                gaussterm = self.reg_term_next[regname] * \
                    reg_term_dict_scatt[regname]
                dgauss_dIa = gaussterm.reshape((N, N))

                # Now the chain rule factor to get the gauss gradient wrt the
                # unscattered image
                gx = (rF**2.0 *
                      so.Wrapped_Convolve(self._ea_ker_gradient_x[::-
                                                                  1, ::-
                                                                  1], phi_Gradient_x *
                                          (dgauss_dIa))).flatten()
                gy = (rF**2.0 *
                      so.Wrapped_Convolve(self._ea_ker_gradient_y[::-
                                                                  1, ::-
                                                                  1], phi_Gradient_y *
                                          (dgauss_dIa))).flatten()

                # Now we add the gradient for the unscattered image
                regterm += so.Wrapped_Convolve(
                    self._ea_ker[::-1, ::-1], (dgauss_dIa)).flatten() + gx + gy
        ...
        # Now the chain rule factor to get the chi^2 gradient wrt the
        # unscattered image
        gx = (rF**2.0 *
              so.Wrapped_Convolve(self._ea_ker_gradient_x[::-
                                                          1, ::-
        ...
        for t in range(1, (N + 1) // 2):
            s = 0
            grad_term = so.Wrapped_Gradient(wavelengthbar /
                                            FOV *
                                            self._sqrtQ[s][t] *
                                            2.0 *
                                            np.cos(2.0 *
            ...
            chisq_grad_epsilon[i_grad] = np.sum(
                dchisq_dIa * rF**2 * (EA_Gradient_x * grad_term_x + EA_Gradient_y * grad_term_y))
            i_grad = i_grad + 1

        # Real part; remainder
        for s in range(1, (N + 1) // 2):
            for t in range(N):
                grad_term = so.Wrapped_Gradient(wavelengthbar / FOV * self._sqrtQ[s][t] * 2.0 * np.cos(
        ...
        for t in range(1, (N + 1) // 2):
            s = 0
            grad_term = so.Wrapped_Gradient(-wavelengthbar / FOV * self._sqrtQ[s][t] * 2.0 * np.sin(
                2.0 * np.pi / N * (ell_mat * s + m_mat * t)) / (FOV / N))
            grad_term_x = -grad_term[1]
            grad_term_y = -grad_term[0]
            chisq_grad_epsilon[i_grad] = np.sum(
        ...
        chisq_epsilon_grad = self.alpha_phi_next * \
            2.0 * EpsilonList / ((N * N - 1) / 2.0)
    ...
    def plotcur_scattering(self, minvec):
        if self._show_updates:
            if self._nit % self._update_interval == 0:
                ...
                for regname in sorted(self.reg_term_next.keys()):
                    regterm += self.reg_term_next[regname] * \
                        reg_term_dict[regname]

                # Scattering screen regularization term
                chisq_epsilon = sum(
                    EpsilonList * EpsilonList) / ((N * N - 1.0) / 2.0)
                regterm_scattering = self.alpha_phi_next * \
                    (chisq_epsilon - 1.0)
    ...
    def _append_image_history(self, outim, logstr):
        ...
        self._systematic_cphase_noise_list.append(
            self.systematic_cphase_noise_next)
