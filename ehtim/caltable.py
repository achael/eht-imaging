...
class Caltable(object):
    ...
    def plot_gains(
        ...
        if axislabels:
            ...
            x.set_ylabel(ylabel)
            plt.title(
                'Caltable gains for %s on day %s' %
                (self.source, self.mjd))
    ...
    def enforce_positive(
        ...
        for site in self.data.keys():
            ...
            if method == 'min':
                sitemin = np.min(
                    [np.abs(self.data[site]['rscale']), np.abs(self.data[site]['lscale'])])
            elif method == 'mean':
                sitemin = np.mean(
                    [np.abs(self.data[site]['rscale']), np.abs(self.data[site]['lscale'])])
            elif method == 'median':
    ...
    def applycal(self, obs, interp='linear', extrapolate=None,
        ...
        if not (self.tarr == obs.tarr).all():
            raise Exception(
                "The telescope array in the Caltable is not the same as in the Obsdata")
        ...
        for s in range(0, len(self.tarr)):
            ...
            rinterp[site] = relaxed_interp1d(
                time_mjd,
                self.data[site]['rscale'],
                kind=interp,
                fill_value=fill_value,
                bounds_error=False)
            linterp[site] = relaxed_interp1d(
                time_mjd,
                self.data[site]['lscale'],
                kind=interp,
                fill_value=fill_value,
                bounds_error=False)
        ...
        for caltable in caltablelist:
            ...
            for site in sites2:
                if site in sites1:  # if site in both tables
                    ...
                    rinterp1 = relaxed_interp1d(
                        time1,
                        data1[site]['rscale'],
                        kind=interp,
                        fill_value=fill_value,
                        bounds_error=False)
                    linterp1 = relaxed_interp1d(
                        time1,
                        data1[site]['lscale'],
                        kind=interp,
                        fill_value=fill_value,
                    ...
                    rinterp2 = relaxed_interp1d(
                        time2,
                        data2[site]['rscale'],
                        kind=interp,
                        fill_value=fill_value,
                        bounds_error=False)
                    linterp2 = relaxed_interp1d(
                        time2,
                        data2[site]['lscale'],
                        kind=interp,
                        fill_value=fill_value,
                    ...
                    rscale_merge = rinterp1(
                        times_merge) * rinterp2(times_merge)
                    lscale_merge = linterp1(
                        times_merge) * linterp2(times_merge)
    ...
    def scan_avg(self, obs, incoherent=True):
        ...
        for s in range(0, ntele):
            ...
            # make a list of times that is the same value for all points in the
            # same scan
            times = self.data[site]['time']
            ...
            for scan in scans:
                ...
                # average the gains
                gains_l_avg = np.mean(
                    gains_l[np.array(times_stable == scan[0])])
                gains_r_avg = np.mean(
                    gains_r[np.array(times_stable == scan[0])])
...
def save_caltable(caltable, obs, datadir='.', sqrt_gains=False):
    ...
    for site_info in caltable.tarr:
        ...
        for entry in site_data:
            ...
            outline = str(float(time)) + ' ' + str(float(rreal)) + ' ' + str(
                float(rimag)) + ' ' + str(float(lreal)) + ' ' + str(float(limag)) + '\n'
...
def plot_tarr_dterms(
    ...
    if export_pdf != "":
        fig, axes = plt.subplots(
            nrows=1, ncols=2, sharey=True, sharex=True, figsize=(
                16, 8))
    else:
...
def plot_compare_gains(
    ...
    # sites
    if sites in ['all' or 'All'] or sites == []:
        sites = list(set(caltab1.data.keys()).intersection(
            caltab2.data.keys()))
