...
# Coordinates in real space are in radian, coordinates in Fourier space
# are in lambda
...
def trianglePulse_F(omega, pdim):
    if (omega == 0):
        ...
        return (4.0 / (pdim**2 * omega**2)) * \
            (math.sin((pdim * omega) / 2.0))**2
...
def cubicPulse_I(x, pdim):
    if abs(x) < pdim:
        ...
        return (-0.5 * (abs(x) / pdim)**3 + 2.5 * (x / pdim)
                ** 2 - 4. * (abs(x) / pdim) + 2.) / pdim
    else:
        return 0.
...
def cubicPulse_F(omega, pdim):
    if (omega == 0):
        ...
        return 2. * ((3. / omega / pdim) * math.sin(omega * pdim / 2.) - math.cos(
            omega * pdim / 2.)) * ((2. / omega / pdim) * math.sin(omega * pdim / 2.))**3
...
#
# return coeff / pdim # TODO : CHECK IF YOU DIVIDE BY PDIM FOR CLUBIC
# SPLINE PULSE
