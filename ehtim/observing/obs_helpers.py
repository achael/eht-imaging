...
def compute_uv_coordinates(
    ...
    if len(site1) == len(site2) == 1:
        ...
        raise Exception(
            "site1, site2, and time not the same dimension in compute_uv_coordinates!")
    ...
    if np.any(spacemask1):
        ...
        for k in range(len(site1space_list)):
            ...
            sat = ephem.readtle(
                array.ephem[site1space][0],
                array.ephem[site1space][1],
                array.ephem[site1space][2])
            ...
            c1 = coords.EarthLocation.from_geodetic(
                lon, lat, elev, ellipsoid=None)
    ...
    if np.any(spacemask2):
        ...
        for k in range(len(site2space_list)):
            ...
            lon = sat.sublong / DEGREE
            # pyephem doesn't use an ellipsoid earth model!
            c2 = coords.EarthLocation.from_geodetic(
                lon, lat, elev, ellipsoid=None)
...
def make_closure_amplitude(
    ...
    if ctype == 'camp':
        ...
        camperr = camp * np.sqrt(1. / (snr1**2) + 1. /
                                 (snr2**2) + 1. / (snr3**2) + 1. / (snr4**2))
        ...
        camp = np.log(np.abs(p1)) + np.log(np.abs(p2)) - \
            np.log(np.abs(p3)) - np.log(np.abs(p4))
        camperr = np.sqrt(1. / (snr1**2) + 1. / (snr2**2) +
                          1. / (snr3**2) + 1. / (snr4**2))
...
def amp_debias(amp, sigma, force_nonzero=False):
    ...
    if force_nonzero:
        deb2 += (np.nan_to_num(np.abs(amp)) <
                 np.nan_to_num(np.abs(sigma))) * np.abs(sigma)**2
    out = np.sqrt(deb2)
...
def logcamp_debias(log_camp, snr1, snr2, snr3, snr4):
    ...
    log_camp_debias = log_camp + 0.5 * \
        (1. / (snr1**2) + 1. / (snr2**2) - 1. / (snr3**2) - 1. / (snr4**2))
...
def merr(sigma, qsigma, usigma, I, m):
    ...
    err = np.sqrt(
        (qsigma**2 + usigma**2 + (sigma * np.abs(m))**2) / (np.abs(I) ** 2))
...
def merr2(rlsigma, rrsigma, llsigma, I, m):
    ...
    err = np.sqrt((rlsigma**2 + (rrsigma**2 + llsigma**2)
                   * np.abs(m)**2) / (np.abs(I) ** 2))
...
def cerror(sigma):
    ...
    noise = np.random.normal(loc=0, scale=sigma) + \
        1j * np.random.normal(loc=0, scale=sigma)
    return noise
...
def image_centroid(im):
    ...
    x0 = np.sum(np.outer(0.0 * ylist + 1.0, xlist).ravel()
                * im.imvec) / np.sum(im.imvec)
    ...
    ftmatrices = [
        ...
        np.outer(
            ...
            np.exp(
                ...
                np.pi *
                xlist *
                uv[0])) for uv in uvlist]  # list of matrices at each freq
...
def ftmatrix_centered(im, pdim, xdim, ydim, uvlist, pulse=PULSE_DEFAULT):
    ...
    # TODO : there is a residual value for the center being around 0, maybe we
    # should chop this off to be exactly 0
    xlist = np.arange(0, -xdim, -1) * pdim + (pdim * xdim) / 2.0 - pdim / 2.0
    ylist = np.arange(0, -ydim, -1) * pdim + (pdim * ydim) / 2.0 - pdim / 2.0
    ...
    ftmatrices = [pulse(2 * np.pi * uv[0],
                        2 * np.pi * uv[1],
                        pdim,
                        dom="F") * np.outer(np.exp(-2j * np.pi * ylist * uv[1]),
                                            np.exp(-2j * np.pi * xlist * uv[0])) for uv in uvlist]
    ftmatrices = np.reshape(np.array(ftmatrices), (len(uvlist), xdim * ydim))
    return ftmatrices
...
def ticks(axisdim, psize, nticks=8):
    ...
    ticklabels = np.around(psize * np.arange((axisdim - 1) /
                                             2.0, -(axisdim) / 2.0, -tickspacing), decimals=1)
...
def utc_to_gmst(utc, mjd):
    ...
    # MJD should always be an integer, but was float in older versions of the
    # code
    mjd = int(mjd)
                ...
                np.dot(
                    np.array(
                        ((np.cos(
                            ...
                            thetas[i]), 0), (np.sin(
                                thetas[i]), np.cos(
                                thetas[i]), 0), (0, 0, 1))), vecs[i]) for i in range(
                                    len(vecs))])
                ...
                np.dot(
                    np.array(
                        ((np.cos(
                            ...
                            thetas[0]), 0), (np.sin(
                                thetas[0]), np.cos(
                                thetas[0]), 0), (0, 0, 1))), vecs[i]) for i in range(
                                    len(vecs))])
                ...
                np.dot(
                    np.array(
                        ((np.cos(
                            ...
                            thetas[i]), 0), (np.sin(
                                thetas[i]), np.cos(
                                thetas[i]), 0), (0, 0, 1))), vecs[0]) for i in range(
                                    len(thetas))])
    else:
        raise Exception(
            "Unequal numbers of vectors and angles in earthrot(vecs, thetas)!")
...
def elev(obsvecs, sourcevec):
    ...
    anglebtw = np.array([np.dot(obsvec, sourcevec) /
                         np.linalg.norm(obsvec) /
...
def par_angle(hr_angle, lat, dec):
    ...
    denom = np.sin(lat) * np.cos(dec) - np.cos(lat) * \
        np.sin(dec) * np.cos(hr_angle)
...
# minimal set as we would from  calling c_phases(count='min'). This is
...
def reduce_tri_minimal(obs, datarr):
    ...
    for timegroup in datalist:
        ...
        sites = list(
            set(np.hstack((timegroup['t1'], timegroup['t2'], timegroup['t3']))))
...
# TODO This returns A minimal set if input is maximal, but it is not necessarily the same
# minimal set as we would from  calling c_amplitudes(count='min'). This is
# because of  inverses.
...
def reduce_quad_minimal(obs, datarr, ctype='camp'):
    ...
    for timegroup in datalist:
        ...
        sites = np.array(list(set(np.hstack(
            (timegroup['t1'], timegroup['t2'], timegroup['t3'], timegroup['t4'])))))
        if len(sites) < 4:
            continue
        ...
        for dp in timegroup:
            ...
            # this is all same closure amplitude, but the ordering of labels is
            # different
...
class NFFTInfo(object):
    def __init__(self, xdim, ydim, psize, pulse, npad, p_rad, uv):
        ...
        pulses = np.fromiter((pulse(2 *
                                    np.pi *
                                    uv_scaled[i, 0], 2 *
                                    np.pi *
                                    uv_scaled[i, 1], 1., dom="F") for i in range(self.uvdim)), 'c16')
...
def sampler(griddata, sampler_info_list, sample_type="vis"):
    ...
    if sample_type not in ["vis", "bs", "camp"]:
        raise Exception(
            "sampler sample_type should be either 'vis','bs',or 'camp'!")
    ...
    for sampler_info in sampler_info_list:
        ...
        datare = nd.map_coordinates(
            np.real(griddata), vu2, order=sampler_info.order)
        dataim = nd.map_coordinates(
            np.imag(griddata), vu2, order=sampler_info.order)
    ...
    for k in range(len(gridder_info_list)):
        ...
        for i in range(2 * p_rad + 1):
            ...
            for j in range(2 * p_rad + 1):
                dx = j - p_rad
                weight = weights[i][j]
                np.add.at(datagrid, tuple(
                    map(tuple, (coords + [dy, dx]).transpose())), data * weight)

    return datagrid


def make_gridder_and_sampler_info(
        im_info,
        uv,
        conv_func=GRIDDER_CONV_FUNC_DEFAULT,
        p_rad=GRIDDER_P_RAD_DEFAULT,
        order=FFT_INTERP_DEFAULT):
    """
    ...
    if not (conv_func in ['pillbox', 'gaussian', 'cubic']):
        raise Exception(
            "conv_func must be either 'pillbox', 'gaussian', or, 'cubic'")
    ...
    # e.g.,
    phase = np.exp(-1j * np.pi * psize * ((1 + im_info.xdim % 2)
                                          * uv[:, 0] + (1 + im_info.ydim % 2) * uv[:, 1]))
    ...
    for i in range(2 * p_rad + 1):
        ...
        for j in range(2 * p_rad + 1):
            ...
            if conv_func == 'gaussian':
                norm = norm + \
                    conv_func_gauss(dy - dcoords[:, 0], dx - dcoords[:, 1])
            elif conv_func == 'pillbox':
                norm = norm + \
                    conv_func_pill(dy - dcoords[:, 0], dx - dcoords[:, 1])
            elif conv_func == 'cubic':
                norm = norm + \
                    conv_func_cubicspline(dy - dcoords[:, 0], dx - dcoords[:, 1])
    ...
    for i in range(2 * p_rad + 1):
        ...
        for j in range(2 * p_rad + 1):
            dx = j - p_rad
            if conv_func == 'gaussian':
                weight = conv_func_gauss(
                    dy - dcoords[:, 0], dx - dcoords[:, 1]) / norm
            elif conv_func == 'pillbox':
                weight = conv_func_pill(
                    dy - dcoords[:, 0], dx - dcoords[:, 1]) / norm
            elif conv_func == 'cubic':
                weight = conv_func_cubicspline(
                    dy - dcoords[:, 0], dx - dcoords[:, 1]) / norm
