...
pl.comp_plots.plotall_obs_compare(
    [obs_unscatt, obs_scatt, obs_scatt_deblur], 'uvdist', 'amp')

# Now approximate the ensemble-average image by averaging 100 realizations
# of the scattering
im_scatt_List = [
    sm.Scatter(
        im,
        so.MakeEpsilonScreen(
            im.xdim,
            im.ydim,
            rngseed=j),
        DisplayImage=False) for j in range(100)]
im_scatt_ea = im.copy()
im_scatt_ea.imvec = np.mean([i.imvec for i in im_scatt_List], axis=0)
im_scatt_ea.display()

# Plot a time series of the total flux density of the scattered image over
# time. One point per hour for a week.
