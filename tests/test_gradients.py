...
# Data types and regularizers to test
...
class TestChisqGradientFiniteDiff:
    ...
    def test_median_frac_diff(self, grad_setup, dtype):
        median_frac, _ = _chisq_gradient_check(grad_setup, dtype)
        assert median_frac < CHISQ_GRAD_MEDIAN_TOL, (
            f"{dtype} median fractional gradient diff = {median_frac:.6f}"
        )

    @pytest.mark.parametrize("dtype", DATATERMS)
    def test_max_frac_diff(self, grad_setup, dtype):
        _, max_frac = _chisq_gradient_check(grad_setup, dtype)
        assert max_frac < CHISQ_GRAD_MAX_TOL, (
            f"{dtype} max fractional gradient diff = {max_frac:.6f}"
        )
...
def _chisq_gradient_check(grad_setup, dtype):
    ...
    cdata = chisqdata(obs, prior, mask, dtype, ttype="direct")
    grad_exact = chisqgrad(test_imvec, cdata[2], cdata[0], cdata[1], dtype, ttype="direct", mask=mask)
    y0 = chisq(test_imvec, cdata[2], cdata[0], cdata[1], dtype, ttype="direct", mask=mask)
    ...
    for i, j in enumerate(sample_idx):
        dx = max(1e-8 * abs(test_imvec[j]), 1e-12)
        imvec2 = test_imvec.copy()
        imvec2[j] += dx
        y1 = chisq(imvec2, cdata[2], cdata[0], cdata[1], dtype, ttype="direct", mask=mask)
...
def _reg_gradient_check(grad_setup, rtype):
    ...
    kwargs = dict(beam_size=20.0 * eh.RADPERUAS, alpha_A=5000.0, norm_reg=True)
