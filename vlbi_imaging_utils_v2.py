...
class Obsdata(object):
    ...
    def save_uvfits(self, fname):
        # !!AC add elevation data
        # !!AC add telescope data
...
def load_obs_uvfits(filename, arrayfile):
    ...
    tarr = load_array(arrayfile).tarr
    ...
    # Sites - give them names
    t1 = data['BASELINE'][mask].astype(int)/256
    t2 = data['BASELINE'][mask].astype(int) - t1*256
    t1 = t1 - 1
    t2 = t2 - 1
    scopes_num = set(np.hstack((t1,t2)))

    
    # !AC is there another way to test if the stations don't match?
    if max(scopes_num) + 1 > len(tarr):
        raise Exception('Array file does not contain all stations in uvfits file!')
...
def add_flat(im, flux):
    ...
    im = (im.imvec + (flux/float(len(im.imvec))) * np.ones(len(im.imvec))).reshape(im.ydim,im.xdim)
    out = Image(im, im.psize, im.ra, im.dec, rf=im.rf, source=im.source, mjd=im.mjd) 
    return out
