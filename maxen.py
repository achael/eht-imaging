...
def maxen_dynamic_bs(Obsdata_List, Prior_List, Flux_List, maxit=100, alpha=100, gamma=500, delta=500, beta=5, beta_blur=5, entropy="simple", stop=1e-10, ipynb=False, refresh_interval = 1000):
	...
	A3 = (vb.ftmatrix(Prior_List[i].psize, Prior_List[i].xdim, Prior_List[i].ydim, uv1),
    ...
    def objfunc(logim):
        ...
        if entropy == "simple":
            s = np.sum(-ssimple(Frames[i].ravel(), nprior_List[i]) for i in range(N_frame))
        elif entropy == "l1":
            s = np.sum(-sl1(Frames[i].ravel(), nprior_List[i]) for i in range(N_frame))
        elif entropy == "gs":
            s = np.sum(-sgs(Frames[i].ravel(), nprior_List[i]) for i in range(N_frame))
        elif entropy == "tv":
            s = np.sum(-stv(Frames[i].ravel(), Prior_List[0].xdim, Prior_List[0].ydim) for i in range(N_frame))

        chisq_total = np.sum(chisq_bi(Frames[i].ravel(), A_List[i], bi_List[i], sigma_List[i]) for i in range(N_frame))/N_frame  
	...
	Frames = np.exp(logim.reshape((-1, N_pixel, N_pixel)))
        if entropy == "simple":
            s = np.concatenate([-ssimplegrad(Frames[i].ravel(), nprior_List[i])*Frames[i].ravel() for i in range(N_frame)])
        elif entropy == "l1":
            s = np.concatenate([-sl1grad(Frames[i].ravel(), nprior_List[i])*Frames[i].ravel() for i in range(N_frame)])
        elif entropy == "gs":
            s = np.concatenate([-sgsgrad(Frames[i].ravel(), nprior_List[i])*Frames[i].ravel() for i in range(N_frame)])
        elif entropy == "tv":
            s = np.concatenate([-stvgrad(Frames[i].ravel(), Prior_List[0].xdim, Prior_List[0].ydim)*Frames[i].ravel() for i in range(N_frame)])
            
	chisq_total_grad = np.concatenate([chisqgrad_bi(Frames[i].ravel(), A_List[i], bi_List[i], sigma_List[i])*Frames[i].ravel()/N_frame for i in range(N_frame)])  
	...
	if nit%10 == 0:        
		...
		chi2 = np.sum(chisq_bi(Frames[i].ravel(), A_List[i], bi_List[i], sigma_List[i]) for i in range(N_frame))/N_frame
		s = np.sum(-ssimple(Frames[i].ravel(), nprior_List[i]) for i in range(N_frame))
		s_dynamic = dynamic_regularizer(Frames, gauss)
		print "chi2: %f" % chi2
		print "s: %f" % s
...
def maxen_dynamic_minimal(Obsdata_List, Prior_List, maxit=100, alpha=100, beta=5, beta_blur=5, entropy="simple", stop=1e-10, ipynb=False, refresh_interval = 1000):
    ...
    def objfunc(logim):
        ...
        chisq_total = np.sum(chisq(Frames[i].ravel(), A_List[i], vis_List[i], sigma_List[i]) for i in range(N_frame))/N_frame  
	...
	chisq_total_grad = np.concatenate([chisqgrad(Frames[i].ravel(), A_List[i], vis_List[i], sigma_List[i])*Frames[i].ravel()/N_frame for i in range(N_frame)])  
	...
	if nit%10 == 0:        
		print "iteration %d" % nit
		Frames = np.exp(logim_step.reshape((-1, N_pixel, N_pixel)))
		chi2 = np.sum(chisq(Frames[i].ravel(), A_List[i], vis_List[i], sigma_List[i]) for i in range(N_frame))/N_frame
		s = np.sum(-ssimple(Frames[i].ravel(), nprior_List[i]) for i in range(N_frame))
		s_dynamic = dynamic_regularizer(Frames, gauss)
		print "chi2: %f" % chi2
		print "s: %f" % s
